...
Для критичного исправления подготовьте PR в `master` и перенесите исправление в `dev` отдельным PR, чтобы оно сохранилось в следующих релизах. Влитие в `master` запускает публикацию.
