...
**Целевая ветка PR — всегда `dev`, не `master`.**

-   feature/fix/chore-ветки отводятся от `dev` и мержатся обратно в `dev`;
-   `master` — релизная ветка; в неё попадают изменения после периодического merge `dev` → `master`;
-   исключение — [HotFix](#hotfix): отдельный PR и в `dev`, и в `master`.

При создании PR в GitHub / `gh pr create` указывайте `--base dev`.
