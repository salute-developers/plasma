...
Следует проверять компоненты на работоспособность в `React.StrictMode`. Для этого в `Storybook` в верхней панели предусмотрен соответствующий механизм включения строгого режима.
