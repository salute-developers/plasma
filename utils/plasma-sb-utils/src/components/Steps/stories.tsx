        ...
        useEffect(() => {
            const newItems = new Array(quantity).fill(null).map((_, index) => {
                ...
                const completedItemView =
                    apllyCustomCompletedItemView && (index < stepItemViews.length ? stepItemViews[index] : 'default');
                const viewTitle = apllyCustomCompletedItemView ? `${completedItemView} view` : innerTitle;
            ...
            setItems(newItems);
        }, [loading, apllyCustomCompletedItemView, quantity, simple]);
