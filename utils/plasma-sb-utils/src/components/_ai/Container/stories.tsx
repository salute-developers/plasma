...
type Attachment = { id: any; type?: 'file' | 'image'; label?: string; size?: string; thumbUrl?: string };
type Message = { id: number; type: 'user' | 'answer'; text: string; isLoading?: boolean; attachments?: Attachment[] };

export const createDefaultStory = (Components: any) => {
    const {
        Container,
        Header,
        Input,
    ...
    const StoryDefault = ({
        ...
        const handleSend = (value: string) => {
            ...
            setTimeout(() => {
                ...
                setMessages((prev) =>
                    prev.map((msg) =>
                        msg.id === answerMsgId ? { ...msg, text: randomResponse, isLoading: false } : msg,
                    ),
                );
