...
export const createDefaultStory = (Answer: any, Components: any) => {
    const { Flow, Button, Chip, ChipGroup, EmbedIconButton } = Components;
    ...
    const StoryDefault = ({ storyContainerWidth, contentVariant, ...args }: any) => {
        return (
            <div style={{ width: `${storyContainerWidth}px`, maxWidth: '100%' }}>
                <Answer
                    {...args}
                    content={contentVariantMap[contentVariant]}
                    footer={<Footer size={args.size ?? 'm'} />}
                />
            </div>
        );
    };
