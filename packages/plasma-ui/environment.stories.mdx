...
Кроме этого к устройствам можно подключиться с помощью приложения Салют и использовать его в качестве пульта. Управление устройствами с помощью приложения Салют возможно как голосовыми командами, так и жестами.
...
Инсеты:  
Панель асистента перекрывает часть WebView. Узнать параметры установленных инсетов можно с помощью действия insets ([AssistantInsetsCommand](https://github.com/salute-developers/salutejs-client#AssistantInsetsCommand)), которое можно получить подписавшись на `assistantClient.onData()`.
