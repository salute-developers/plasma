...
Каждая переменная имеет описание в виде комментария. Современные `IDE` будут выводить этот комментарий в виде подсказки.
...
Текст – это основа любого интерфейса.

Для того чтобы текст выглядел единообразно во всей экосистеме `Salute`, разработана данная типографическая система.
...
-   Размер шрифта выглядит хорошо на desktops устройстве, но будет мелковат на ТВ-платформе.
...
Его необходимо добавить внутрь тега `head`.
...
Для остальных поверхностей предполагается использовать значение по умолчанию: `16px`.
...
Цвета можно забирать из корня пакета, для каждого цвета есть комментарий описывающий его использование.
...
## Создание custom theme

```bash
npm run create:theme
```

В результате в директории `data/themes` будут созданы файлы:

-   `<name>.json` - файл темы
-   `<name>.html` - визуальное представление темы

В случае если файл темы `<name>.json` был изменен вручную, необходимо выполнить команду:

```bash
"generate:theme-html": "npx ts-node ./lib/themeBuilder/generateThemeHTML"
```

В результате рядом с каждым файлом темы в директории `data/themes` будет создан или обновлен `html` файл темы.
