...
import { VirtualCarouselItemProps } from '@salutejs/plasma-core';

import { MusicCard } from '../Card/Card.examples';

import { VirtualCarouselCol } from '.';
...
export interface ScalingColCardProps extends Omit<VirtualCarouselItemProps, 'size' | 'sizeM'> {
    isActive: boolean;
    item: {
        title: string;
        imageSrc: string;
...
export const ScalingColCard: React.FC<ScalingColCardProps> = ({ isActive, scrollSnapAlign, item, ...rest }) => (
    <VirtualCarouselCol size={2} sizeM={1.5} scrollSnapAlign={scrollSnapAlign} {...rest}>
        <StyledColInner>
            <StyledMusicCard
                title={item.title}
                focused={isActive}
