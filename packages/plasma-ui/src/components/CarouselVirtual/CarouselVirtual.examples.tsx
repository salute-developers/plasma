...
import { CarouselVirtualItemProps, CarouselVirtualCol } from '@salutejs/plasma-core';
...
export interface ScalingColCardProps extends Omit<CarouselVirtualItemProps, 'size' | 'sizeM'> {
    isActive: boolean;
    item: {
        title: string;
        imageSrc: string;
...
export const ScalingColCard: React.FC<ScalingColCardProps> = ({ isActive, scrollSnapAlign, item, ...rest }) => (
    <CarouselVirtualCol size={2} sizeM={1.5} scrollSnapAlign={scrollSnapAlign} {...rest}>
        <StyledColInner>
            <StyledMusicCard
                title={item.title}
                focused={isActive}
