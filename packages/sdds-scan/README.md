# Библиотека компонентов для вертикали SDDS SCAN
...
[![npm version](https://img.shields.io/npm/v/@salutejs/sdds-scan.svg)](https://www.npmjs.com/package/@salutejs/sdds-scan)
...
[![sdds-themes](https://img.shields.io/github/package-json/dependency-version/salute-developers/plasma/@salutejs/sdds-themes?filename=packages%2Fsdds-scan%2Fpackage.json)](https://www.npmjs.com/package/@salutejs/sdds-themes)
...
-   [Документация](https://plasma.sberdevices.ru/sdds-scan/) — гайды, список компонентов с примерами и API
-   [Подключение в Next.js](https://plasma.sberdevices.ru/sdds-scan/next/)
-   [Changelog](https://plasma.sberdevices.ru/changelog/?vertical=SDDSScan&platform=React&version=0.367.0)
...
| ----------------------------------------- | ----------------- | --------------------------- |
| `@salutejs/sdds-scan`                     | предсобранный CSS | нет                          |
| `@salutejs/sdds-scan/styled-components`   | styled-components | `styled-components@5.3.1`    |
...
Доступны светлая и тёмная темы — `sdds_scan__light` и `sdds_scan__dark`.

### Через импорт css-файла

```jsx
import '@salutejs/sdds-themes/css/sdds_scan__light.css';
```
...
Переключение темы в рантайме и подключение через css-модули описаны в [документации](https://plasma.sberdevices.ru/sdds-scan/).
...
-   уникальные токены темы — из `@salutejs/sdds-themes/tokens/sdds_scan`.
...
```js
const nextConfig = {
    transpilePackages: ['@salutejs/sdds-scan', '@salutejs/plasma-new-hope', '@salutejs/plasma-icons'],
};
```

Подробности — в [гайде по Next.js](https://plasma.sberdevices.ru/sdds-scan/next/).

## MCP-сервер для AI-агентов

> **Примечание**: Поддерживаются версии библиотеки начиная с `0.349.0` — нужная указывается параметром `--version`.

Библиотека предоставляет [MCP-сервер](https://plasma.sberdevices.ru/sdds-scan/how-to-mcp/) `@salutejs/sdds-mcp` — через него LLM-агент (Claude Code, Cursor и др.) получает актуальную документацию:
...
```bash
npx -y @salutejs/sdds-mcp@latest --lib sdds-scan
```
...
```json
{
    "mcpServers": {
        "sdds-scan": {
            "command": "npx",
            "args": ["-y", "@salutejs/sdds-mcp@latest", "--lib", "sdds-scan"]
        }
    }
}
```

Подробности и список инструментов — в [гайде по MCP](https://plasma.sberdevices.ru/sdds-scan/how-to-mcp/).
