...
import { H2, Button } from '@salutejs/sdds-bizcom';

export default function Home() {
    return (
        <>
            <Head>
                <title>Create Next App with sdds-bizcom components</title>
                <link
                    rel="stylesheet"
...
```css
@import '@salutejs/sdss-themes/css/sdds_bizcom__dark.css';
```
...
-   Непосредственно из темы `@salutejs/sdds-themes/tokens/sdds-bizcom` (следует использовать, когда необходимо импортировать уникальные токены, которые используются только в этой теме)
...
```ts
import { BodyL, DsplL, H3 } from '@salutejs/sdds-bizcom';
```
