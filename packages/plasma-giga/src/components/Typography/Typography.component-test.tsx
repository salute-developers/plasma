...
describe('plasma-giga: typograph', () => {
    ...
    describe('URL', () => {
        ...
        it('не забирает закрывающую кавычку из обёртки URL', () => {
            ...
            expect(typograph('"См. https://example.test"')).to.equal('«См. https://example.test»');
            expect(typograph('он сказал "см https://example.test" вчера')).to.equal(
                `он${NBSP}сказал «см${NBSP}https://example.test» вчера`,
            );
        });
