...
const StoryCard = (props: TourProps) => {
    ...
    const renderTourCard = (stepCurrent: number, stepLength: number, last: boolean, step: any) => {
        const actionButtons = props.showActionButtons ? (
            <ButtonGroup stretching="auto" gap="wide" isCommonButtonStyles={false}>
                {stepCurrent > 0 && (
                ...
                {last ? (
                    <Button size="s" view="secondary" onClick={onClose}>
                        Закрыть
                    </Button>
                ) : (
                    <Button size="s" view="accent" onClick={() => setCurrent((c) => c + 1)}>
                        Далее
                    </Button>
                )}
            </ButtonGroup>
        ...
        const skipButton = props.showSkipButton ? (
            <Button view="clear" size="s" onClick={onClose}>
                Пропустить
            </Button>
        ) : undefined;
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Button id="button-id" view="accent">
                    Сохранить
                </Button>
            </div>
