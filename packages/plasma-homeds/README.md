...
Так же библиотека поставляет компоненты собранные с помощью css
...
import { H2, Button } from '@salutejs/plasma-homeds';

export default function Home() {
    return (
        <>
            <Head>
                <title>Create Next App with HomeDS components</title>
                <link
                    rel="stylesheet"
...
-   Непосредственно из темы `@salutejs/plasma-themes/tokens/plasma-homeds` (следует использовать, когда необходимо импортировать уникальные токены, которые используются только в этой теме)
...
```ts
import { BodyL, DsplL, H3 } from '@salutejs/plasma-homeds';
```
