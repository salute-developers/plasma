...
const getShadow = (shadow: any, tokens: TokenType[], direction: 'down' | 'up' | 'gradient', isJS = false) =>
    tokens
...
export const createShadowTokens = (shadow: any, tokens?: Array<TokenType>, isJS = false) => {
    ...
    const shadows = [
        getShadow(shadow, tokens, 'down', isJS),
        getShadow(shadow, tokens, 'up', isJS),
        getShadow(shadow, tokens, 'gradient', isJS),
    ]
        .filter(Boolean)
        .join('\n');
