...
import { Table } from '@salutejs/{{ package }}';
...
<Tabs>
    ...
    <TabItem value="allInOne" label="All in one">
        ```tsx live
        import React from 'react';
        import { Table, Button, ButtonGroup, IconButton, Dropdown } from '@salutejs/{{ package }}';
...
```tsx live
    ...
    export function App() {
        ...
        const [rowHighlighted, setTowHighlighted] = React.useState(null);

        const setRowProps = (row: any) => {
            return {
                onClick: () => setTowHighlighted(row.id),
