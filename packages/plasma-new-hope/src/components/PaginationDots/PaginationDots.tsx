        ...
        (
            ...
            useEffect(() => {
                ...
                console.log('###', {
                    ...
                    b: canScrollBackward,
                    iv: innerViewValue,
                    ivc: innerVisibleCount,
                    d: innerVisibleCount - innerViewValue,
                });
            }, [value, offset, innerVisibleCount, count]);
            ...
            /*
                        useLayoutEffect(() => {
                            ...
                            if (!innerRef?.current || !centered) {
                                return;
                            }

                            prevHeight.current = height.current;
                            height.current = getHeight(innerRef.current);

                            const innerViewValue = value - offset;

                            const children = Array.from(innerRef.current.children);
                            const currentSize = children[innerViewValue].offsetHeight;

                            const beforeItemsHeight = children
                                .splice(0, innerViewValue)
                                .reduce((res, child) => (res += child.offsetHeight), 0);
                            const nextItemsHeight = children
                                .splice(innerViewValue + 1)
                                .reduce((res, child) => (res += child.offsetHeight), 0);

                            // marginTop.current = `${-beforeItemsHeight + nextItemsHeight}px`;

                            children[0].style.marginTop = `${nextItemsHeight}px`;
                            console.log(
                                '!!!',

                                JSON.stringify({
                                    i: innerViewValue,
                                    ph: prevHeight.current,
                                    h: height.current,
                                    cs: currentSize,
                                    b: beforeItemsHeight,
                                    n: nextItemsHeight,
            ...
            */
            ...
            const dotsNodes = dots.map((_, i) => {
                ...
                const dotSize = getDotSize(size, index, currentValue, offset, count, visibleCount);
