...
const parseTimeSegments = (
    ...
    const explicitMeridiem = input.match(/(?:^|[^a-z])(AM|PM)\s*$/i)?.[1].toUpperCase() as Meridiem | undefined;
    const digits = input.replace(/\D/g, '');
    ...
    return { values, segments, text: segments.filter(Boolean).join(delimiter), meridiem: explicitMeridiem ?? meridiem };
};
...
export const normalize12HourSegment = (input: string): NormalizedSegment => {
    ...
    if (digits.length === 1) {
        if (num <= 1) {
            return { text: digits, value: num === 0 ? null : num, carryover: '' };
        }
...
/**
 * Разбор ввода в 12-часовом формате. AM/PM задаётся колонкой в списке или при вставке текста.
 */
