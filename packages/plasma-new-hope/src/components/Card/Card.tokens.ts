export const classes = {};

export const tokens = {
    borderWidth: '--plasma-card-border-width',
    borderRadius: '--plasma-card-border-radius',
    contentBorderRadius: '--plasma-card-content-border-radius',
    outlineWidth: '--plasma-card-outline-width',

    titleColor: '--plasma-card-title-color',
    solidBackground: '--plasma-card-solid-background',
    colorBackground: '--plasma-card-color-background',

    titleFontFamily: '--plasma-step-item-title-font-family',
    titleFontSize: '--plasma-step-item-title-font-size',
    titleFontStyle: '--plasma-step-item-title-font-style',
    titleFontWeight: '--plasma-step-item-title-font-weight',
    titleLetterSpacing: '--plasma-step-item-title-letter-spacing',
    titleLineHeight: '--plasma-step-item-title-line-height',
};
