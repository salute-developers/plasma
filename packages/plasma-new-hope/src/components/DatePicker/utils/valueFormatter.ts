...
const handleRemoveSymbol = ({
    ...
    if (previousValue?.charAt(selectionStart) === delimiter) {
        if (previousValue?.charAt(secondDelimiterIndex) === delimiter || !previousValue?.charAt(secondDelimiterIndex)) {
            return {
...
export const formatValueOnInput = ({
    ...
    if (currentKeyPressed === 'Backspace') {
        return handleRemoveSymbol({
            ...
            previousValue,
            selectionStart,
            secondDelimiterIndex: selectionStart + 1,
    ...
    if (currentFormatPart.type === dateSymbolsMap.M && !currentFormatPart?.isNumeric) {
        ...
        newSelectionStart === endOfDatePart
    ) {
        if (
            !isDateItemEntered(currentFormatPart, prevDateParts[changedPartIndex], lang) &&
            !formattedDate.slice(startOfNextDatePart).charAt(0) &&
