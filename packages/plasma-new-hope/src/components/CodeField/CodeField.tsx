...
export const codeFieldRoot = (Root: RootProps<HTMLDivElement, CodeFieldProps>) =>
    forwardRef<HTMLInputElement, CodeFieldProps>(
        (
            ...
            const [code, setCode] = useState(getCodeValue(codeLength, outerValue || ''));
            const [originalValue, setOriginalValue] = useState(code.join(''));
            ...
            useEffect(() => {
                if (onFullCodeEnter && originalValue.length === codeLength) {
                    onFullCodeEnter(originalValue);
                }
            }, [originalValue, onFullCodeEnter]);
            ...
            return (
                ...
                >
                                    ...
                                    {[...Array(codeLength / parts)].map((_, i) => {
                                        ...
                                        return (
                                            <ItemInput
                                                ...
                                                className={cls({ [classes.segmented]: shape === 'segmented' })}
                                                value={code[inputCorrectIndex] || ''}
                                                autoComplete={autoComplete}
                                                onChange={(e) => handleChange(e, inputCorrectIndex)}
                                                onKeyDown={(e) => handleOnKeyDown(e, inputCorrectIndex)}
                                                onPaste={handlePaste}
                    ...
                    {caption && (
                        <CaptionWrapper ref={captionRef} captionAlign={captionAlign}>
                            {caption}
                        </CaptionWrapper>
                    )}
                </Root>
