// Generated by robots, do not change this manually!

export const sdds_insol__light = {
    ':root': {
        '--text-primary': '#13181BF5',
        '--text-primary-hover': '#13181B7A',
        '--text-primary-active': '#13181BFF',
        '--text-primary-brightness': '#13181BF5',
        '--text-secondary': '#13181B8F',
        '--text-secondary-hover': '#13181B47',
        '--text-secondary-active': '#13181BFF',
        '--text-tertiary': '#13181B47',
        '--text-tertiary-hover': '#13181B2B',
        '--text-tertiary-active': '#13181B72',
        '--text-paragraph': '#13181BCC',
        '--text-paragraph-hover': '#13181B66',
        '--text-paragraph-active': '#13181BFF',
        '--text-accent': '#108E26',
        '--text-accent-hover': '#15BC31FF',
        '--text-accent-active': '#0C6A1BFF',
        '--text-accent-gradient-hover': '#CCCCCCFF',
        '--text-accent-gradient-active': '#E6E6E6FF',
        '--text-accent-minor': '#28D247',
        '--text-accent-minor-hover': '#7AE68EFF',
        '--text-accent-minor-active': '#23B83EFF',
        '--text-positive': '#108E26',
        '--text-positive-hover': '#15BC31FF',
        '--text-positive-active': '#0C6A1BFF',
        '--text-warning': '#E85702',
        '--text-warning-hover': '#FD9253FF',
        '--text-warning-active': '#CA4B02FF',
        '--text-negative': '#F31B31',
        '--text-negative-hover': '#F87C89FF',
        '--text-negative-active': '#E40C22FF',
        '--text-info': '#0B7ECB',
        '--text-info-hover': '#47B0F5FF',
        '--text-info-active': '#096CAEFF',
        '--text-positive-minor': '#28D247',
        '--text-positive-minor-hover': '#7AE68EFF',
        '--text-positive-minor-active': '#23B83EFF',
        '--text-warning-minor': '#FD9C68',
        '--text-warning-minor-hover': '#FDB790FF',
        '--text-warning-minor-active': '#FC8240FF',
        '--text-negative-minor': '#FF8F9A',
        '--text-negative-minor-hover': '#FFB8BFFF',
        '--text-negative-minor-active': '#FF6675FF',
        '--text-info-minor': '#52BAFF',
        '--text-info-minor-hover': '#A3DAFFFF',
        '--text-info-minor-active': '#29A9FFFF',
        '--text-accent-gradient': 'linear-gradient(45deg, #0D8523 0%, #0DA8AB 100%)',
        '--on-dark-text-primary-hover': '#F6F6F97A',
        '--on-dark-text-primary-active': '#F6F6F9FF',
        '--on-dark-text-primary-brightness': '#F6F6F9F5',
        '--on-dark-text-secondary-hover': '#F6F6F947',
        '--on-dark-text-secondary-active': '#F6F6F9FF',
        '--on-dark-text-tertiary-hover': '#F6F6F92B',
        '--on-dark-text-tertiary-active': '#F6F6F972',
        '--on-dark-text-paragraph-hover': '#F6F6F966',
        '--on-dark-text-paragraph-active': '#F6F6F9FF',
        '--on-dark-text-accent': '#24B23E',
        '--on-dark-text-accent-hover': '#5EDE76FF',
        '--on-dark-text-accent-active': '#1F9835FF',
        '--on-dark-text-accent-gradient-hover': '#CCCCCCFF',
        '--on-dark-text-accent-gradient-active': '#E6E6E6FF',
        '--on-dark-text-accent-minor': '#095C18',
        '--on-dark-text-accent-minor-hover': '#0F9527FF',
        '--on-dark-text-accent-minor-active': '#074A14FF',
        '--on-dark-text-positive': '#24B23E',
        '--on-dark-text-positive-hover': '#5EDE76FF',
        '--on-dark-text-positive-active': '#1F9835FF',
        '--on-dark-text-warning': '#FF7024',
        '--on-dark-text-warning-hover': '#FFA575FF',
        '--on-dark-text-warning-active': '#FA5700FF',
        '--on-dark-text-negative': '#FF3D51',
        '--on-dark-text-negative-hover': '#FF8F9AFF',
        '--on-dark-text-negative-active': '#FF142CFF',
        '--on-dark-text-info': '#199AF0',
        '--on-dark-text-info-hover': '#79C4F6FF',
        '--on-dark-text-info-active': '#0E8ADDFF',
        '--on-dark-text-positive-minor': '#095C18',
        '--on-dark-text-positive-minor-hover': '#0F9527FF',
        '--on-dark-text-positive-minor-active': '#074A14FF',
        '--on-dark-text-warning-minor': '#85380C',
        '--on-dark-text-warning-minor-hover': '#B14B10FF',
        '--on-dark-text-warning-minor-active': '#5D2809FF',
        '--on-dark-text-negative-minor': '#9C1422',
        '--on-dark-text-negative-minor-hover': '#DD1D30FF',
        '--on-dark-text-negative-minor-active': '#8C121EFF',
        '--on-dark-text-info-minor': '#0D5382',
        '--on-dark-text-info-minor-hover': '#137DC3FF',
        '--on-dark-text-info-minor-active': '#093B5DFF',
        '--on-dark-text-secondary': 'rgba(245,245,248,0.56)',
        '--on-dark-text-tertiary': 'rgba(245,245,248,0.28)',
        '--on-dark-text-paragraph': 'rgba(245,245,248,0.8)',
        '--on-dark-text-primary': 'rgba(245,245,248,0.96)',
        '--on-dark-text-accent-gradient': 'linear-gradient(45deg, #1A9E32 0%, #04C6C9 100%)',
        '--on-light-text-primary': '#13181BF5',
        '--on-light-text-primary-hover': '#13181B7A',
        '--on-light-text-primary-active': '#13181BFF',
        '--on-light-text-primary-brightness': '#13181BF5',
        '--on-light-text-secondary': '#13181B8F',
        '--on-light-text-secondary-hover': '#13181B47',
        '--on-light-text-secondary-active': '#13181BFF',
        '--on-light-text-tertiary': '#13181B47',
        '--on-light-text-tertiary-hover': '#13181B2B',
        '--on-light-text-tertiary-active': '#13181B72',
        '--on-light-text-paragraph': '#13181BCC',
        '--on-light-text-paragraph-hover': '#13181B66',
        '--on-light-text-paragraph-active': '#13181BFF',
        '--on-light-text-accent': '#108E26',
        '--on-light-text-accent-hover': '#15BC31FF',
        '--on-light-text-accent-active': '#0C6A1BFF',
        '--on-light-text-accent-gradient-hover': '#CCCCCCFF',
        '--on-light-text-accent-gradient-active': '#E6E6E6FF',
        '--on-light-text-accent-minor': '#28D247',
        '--on-light-text-accent-minor-hover': '#7AE68EFF',
        '--on-light-text-accent-minor-active': '#23B83EFF',
        '--on-light-text-positive': '#108E26',
        '--on-light-text-positive-hover': '#15BC31FF',
        '--on-light-text-positive-active': '#0C6A1BFF',
        '--on-light-text-warning': '#E85702',
        '--on-light-text-warning-hover': '#FD9253FF',
        '--on-light-text-warning-active': '#CA4B02FF',
        '--on-light-text-negative': '#F31B31',
        '--on-light-text-negative-hover': '#F87C89FF',
        '--on-light-text-negative-active': '#E40C22FF',
        '--on-light-text-info': '#0B7ECB',
        '--on-light-text-info-hover': '#47B0F5FF',
        '--on-light-text-info-active': '#096CAEFF',
        '--on-light-text-positive-minor': '#28D247',
        '--on-light-text-positive-minor-hover': '#7AE68EFF',
        '--on-light-text-positive-minor-active': '#23B83EFF',
        '--on-light-text-warning-minor': '#FD9C68',
        '--on-light-text-warning-minor-hover': '#FDB790FF',
        '--on-light-text-warning-minor-active': '#FC8240FF',
        '--on-light-text-negative-minor': '#FF8F9A',
        '--on-light-text-negative-minor-hover': '#FFB8BFFF',
        '--on-light-text-negative-minor-active': '#FF6675FF',
        '--on-light-text-info-minor': '#52BAFF',
        '--on-light-text-info-minor-hover': '#A3DAFFFF',
        '--on-light-text-info-minor-active': '#29A9FFFF',
        '--on-light-text-accent-gradient': 'linear-gradient(45deg, #0D8523 0%, #0DA8AB 100%)',
        '--inverse-text-primary-hover': '#F6F6F97A',
        '--inverse-text-primary-active': '#F6F6F9FF',
        '--inverse-text-primary-brightness': '#F6F6F9F5',
        '--inverse-text-secondary-hover': '#F6F6F947',
        '--inverse-text-secondary-active': '#F6F6F9FF',
        '--inverse-text-tertiary-hover': '#F6F6F92B',
        '--inverse-text-tertiary-active': '#F6F6F972',
        '--inverse-text-paragraph-hover': '#F6F6F966',
        '--inverse-text-paragraph-active': '#F6F6F9FF',
        '--inverse-text-accent': '#24B23E',
        '--inverse-text-accent-hover': '#5EDE76FF',
        '--inverse-text-accent-active': '#1F9835FF',
        '--inverse-text-accent-gradient': '#FFFFFF',
        '--inverse-text-accent-gradient-hover': '#CCCCCCFF',
        '--inverse-text-accent-gradient-active': '#E6E6E6FF',
        '--inverse-text-accent-minor': '#095C18',
        '--inverse-text-accent-minor-hover': '#0F9527FF',
        '--inverse-text-accent-minor-active': '#074A14FF',
        '--inverse-text-positive': '#24B23E',
        '--inverse-text-positive-hover': '#5EDE76FF',
        '--inverse-text-positive-active': '#1F9835FF',
        '--inverse-text-warning': '#FF7024',
        '--inverse-text-warning-hover': '#FFA575FF',
        '--inverse-text-warning-active': '#FA5700FF',
        '--inverse-text-negative': '#FF3D51',
        '--inverse-text-negative-hover': '#FF8F9AFF',
        '--inverse-text-negative-active': '#FF142CFF',
        '--inverse-text-info': '#199AF0',
        '--inverse-text-info-hover': '#79C4F6FF',
        '--inverse-text-info-active': '#0E8ADDFF',
        '--inverse-text-positive-minor': '#095C18',
        '--inverse-text-positive-minor-hover': '#0F9527FF',
        '--inverse-text-positive-minor-active': '#074A14FF',
        '--inverse-text-warning-minor': '#85380C',
        '--inverse-text-warning-minor-hover': '#B14B10FF',
        '--inverse-text-warning-minor-active': '#5D2809FF',
        '--inverse-text-negative-minor': '#9C1422',
        '--inverse-text-negative-minor-hover': '#DD1D30FF',
        '--inverse-text-negative-minor-active': '#8C121EFF',
        '--inverse-text-info-minor': '#0D5382',
        '--inverse-text-info-minor-hover': '#137DC3FF',
        '--inverse-text-info-minor-active': '#093B5DFF',
        '--inverse-text-primary': 'rgba(245,245,248,0.96)',
        '--inverse-text-secondary': 'rgba(245,245,248,0.56)',
        '--inverse-text-tertiary': 'rgba(245,245,248,0.28)',
        '--inverse-text-paragraph': 'rgba(245,245,248,0.8)',
        '--surface-solid-primary': '#F2F5F8',
        '--surface-solid-primary-hover': '#EEF2F6FF',
        '--surface-solid-primary-active': '#EBF0F4FF',
        '--surface-solid-primary-brightness': '#FFFFFFFF',
        '--surface-solid-secondary': '#E8EEF2',
        '--surface-solid-secondary-hover': '#E2E9EEFF',
        '--surface-solid-secondary-active': '#DEE7EDFF',
        '--surface-solid-tertiary': '#D5DFE6',
        '--surface-solid-tertiary-hover': '#CFDAE2FF',
        '--surface-solid-tertiary-active': '#C9D5DEFF',
        '--surface-solid-card': '#FFFFFF',
        '--surface-solid-card-hover': '#FFFFFFFF',
        '--surface-solid-card-active': '#FFFFFFFF',
        '--surface-solid-card-brightness': '#FFFFFFFF',
        '--surface-solid-default': '#060A0C',
        '--surface-solid-default-hover': '#111C22FF',
        '--surface-solid-default-active': '#030607FF',
        '--surface-transparent-primary-hover': '#070B0E0A',
        '--surface-transparent-primary-active': '#070B0E0D',
        '--surface-transparent-secondary-hover': '#070B0E14',
        '--surface-transparent-secondary-active': '#070B0E1A',
        '--surface-transparent-tertiary': '#060A0C1F',
        '--surface-transparent-tertiary-hover': '#070B0E2E',
        '--surface-transparent-tertiary-active': '#070B0E33',
        '--surface-transparent-deep': '#060A0CA3',
        '--surface-transparent-deep-hover': '#070B0EB3',
        '--surface-transparent-deep-active': '#070B0EB8',
        '--surface-transparent-card': 'rgba(255, 255, 255, 0.6)',
        '--surface-transparent-card-hover': '#FFFFFFA8',
        '--surface-transparent-card-active': '#FFFFFFAD',
        '--surface-transparent-card-brightness': '#FFFFFF99',
        '--surface-clear': '#FFFFFF00',
        '--surface-clear-hover': '#FFFFFFFF',
        '--surface-clear-active': '#FFFFFFFF',
        '--surface-accent': '#1A9E32',
        '--surface-accent-hover': '#1EB83AFF',
        '--surface-accent-active': '#178C2CFF',
        '--surface-accent-gradient-hover': '#FFFFFFFF',
        '--surface-accent-gradient-active': '#FFFFFFFF',
        '--surface-accent-minor': '#9EFAAF',
        '--surface-accent-minor-hover': '#94F9A7FF',
        '--surface-accent-minor-active': '#8BF99FFF',
        '--surface-transparent-accent': '#1A9E321F',
        '--surface-transparent-accent-hover': '#1A9E322E',
        '--surface-transparent-accent-active': '#1A9E3233',
        '--surface-positive': '#1A9E32',
        '--surface-positive-hover': '#1EB83AFF',
        '--surface-positive-active': '#178C2CFF',
        '--surface-warning': '#FA5F05',
        '--surface-warning-hover': '#FB782DFF',
        '--surface-warning-active': '#E65705FF',
        '--surface-negative': '#FF293E',
        '--surface-negative-hover': '#FF5263FF',
        '--surface-negative-active': '#FF142CFF',
        '--surface-info': '#118CDF',
        '--surface-info-hover': '#209CEEFF',
        '--surface-info-active': '#0F81CCFF',
        '--surface-positive-minor': '#9EFAAF',
        '--surface-positive-minor-hover': '#94F9A7FF',
        '--surface-positive-minor-active': '#8BF99FFF',
        '--surface-warning-minor': '#FEE2D2',
        '--surface-warning-minor-hover': '#FEDCC8FF',
        '--surface-warning-minor-active': '#FED9C3FF',
        '--surface-negative-minor': '#FFE0E3',
        '--surface-negative-minor-hover': '#FFD6DAFF',
        '--surface-negative-minor-active': '#FFD1D6FF',
        '--surface-info-minor': '#CFECFF',
        '--surface-info-minor-hover': '#C7E9FFFF',
        '--surface-info-minor-active': '#C2E7FFFF',
        '--surface-transparent-positive': '#1A9E321F',
        '--surface-transparent-positive-hover': '#1A9E322E',
        '--surface-transparent-positive-active': '#1A9E3233',
        '--surface-transparent-warning': '#FA5F051F',
        '--surface-transparent-warning-hover': '#FA5F052E',
        '--surface-transparent-warning-active': '#FA5F0533',
        '--surface-transparent-negative': '#FF293E1F',
        '--surface-transparent-negative-hover': '#FF293E2E',
        '--surface-transparent-negative-active': '#FF293E33',
        '--surface-transparent-info': '#118CDF1F',
        '--surface-transparent-info-hover': '#118CDF2E',
        '--surface-transparent-info-active': '#118CDF33',
        '--surface-transparent-primary': 'rgba(6,10,12,0.03)',
        '--surface-transparent-secondary': 'rgba(6,10,12,0.06)',
        '--surface-accent-gradient': 'linear-gradient(45deg, #15B015 0%, #04C6C9 100%)',
        '--on-dark-surface-solid-primary': '#13181B',
        '--on-dark-surface-solid-primary-hover': '#20282DFF',
        '--on-dark-surface-solid-primary-active': '#0B0D0FFF',
        '--on-dark-surface-solid-primary-brightness': '#20282DFF',
        '--on-dark-surface-solid-secondary': '#23292D',
        '--on-dark-surface-solid-secondary-hover': '#2C343AFF',
        '--on-dark-surface-solid-secondary-active': '#1B1F23FF',
        '--on-dark-surface-solid-tertiary': '#30373C',
        '--on-dark-surface-solid-tertiary-hover': '#394147FF',
        '--on-dark-surface-solid-tertiary-active': '#272C30FF',
        '--on-dark-surface-solid-card': '#13181B',
        '--on-dark-surface-solid-card-hover': '#20282DFF',
        '--on-dark-surface-solid-card-active': '#0B0D0FFF',
        '--on-dark-surface-solid-card-brightness': '#20282DFF',
        '--on-dark-surface-solid-default': '#F7F9FB',
        '--on-dark-surface-solid-default-hover': '#F5F7FAFF',
        '--on-dark-surface-solid-default-active': '#F1F5F8FF',
        '--on-dark-surface-transparent-primary': 'rgba(255, 255, 255, 0.06)',
        '--on-dark-surface-transparent-primary-hover': '#FFFFFF14',
        '--on-dark-surface-transparent-primary-active': '#FFFFFF1A',
        '--on-dark-surface-transparent-secondary': 'rgba(255, 255, 255, 0.12)',
        '--on-dark-surface-transparent-secondary-hover': '#FFFFFF2E',
        '--on-dark-surface-transparent-secondary-active': '#FFFFFF33',
        '--on-dark-surface-transparent-tertiary': 'rgba(255, 255, 255, 0.2)',
        '--on-dark-surface-transparent-tertiary-hover': '#FFFFFF42',
        '--on-dark-surface-transparent-tertiary-active': '#FFFFFF47',
        '--on-dark-surface-transparent-deep': 'rgba(255, 255, 255, 0.64)',
        '--on-dark-surface-transparent-deep-hover': '#FFFFFFB3',
        '--on-dark-surface-transparent-deep-active': '#FFFFFFB8',
        '--on-dark-surface-transparent-card': 'rgba(255, 255, 255, 0.06)',
        '--on-dark-surface-transparent-card-hover': '#FFFFFF14',
        '--on-dark-surface-transparent-card-active': '#FFFFFF1A',
        '--on-dark-surface-transparent-card-brightness': '#FFFFFF0F',
        '--on-dark-surface-clear': '#FFFFFF00',
        '--on-dark-surface-clear-hover': '#FFFFFFFF',
        '--on-dark-surface-clear-active': '#FFFFFFFF',
        '--on-dark-surface-accent': '#1A9E32',
        '--on-dark-surface-accent-hover': '#1EB83AFF',
        '--on-dark-surface-accent-active': '#178C2CFF',
        '--on-dark-surface-accent-gradient-hover': '#FFFFFFFF',
        '--on-dark-surface-accent-gradient-active': '#FFFFFFFF',
        '--on-dark-surface-accent-minor': '#0A2B10',
        '--on-dark-surface-accent-minor-hover': '#0E3A16FF',
        '--on-dark-surface-accent-minor-active': '#061909FF',
        '--on-dark-surface-transparent-accent': '#1A9E3233',
        '--on-dark-surface-transparent-accent-hover': '#1A9E3242',
        '--on-dark-surface-transparent-accent-active': '#1A9E3247',
        '--on-dark-surface-positive': '#1A9E32',
        '--on-dark-surface-positive-hover': '#1EB83AFF',
        '--on-dark-surface-positive-active': '#178C2CFF',
        '--on-dark-surface-warning': '#FA5F05',
        '--on-dark-surface-warning-hover': '#FB782DFF',
        '--on-dark-surface-warning-active': '#E65705FF',
        '--on-dark-surface-negative': '#FF293E',
        '--on-dark-surface-negative-hover': '#FF5263FF',
        '--on-dark-surface-negative-active': '#FF142CFF',
        '--on-dark-surface-info': '#118CDF',
        '--on-dark-surface-info-hover': '#209CEEFF',
        '--on-dark-surface-info-active': '#0F81CCFF',
        '--on-dark-surface-positive-minor': '#0A2B10',
        '--on-dark-surface-positive-minor-hover': '#0E3A16FF',
        '--on-dark-surface-positive-minor-active': '#061909FF',
        '--on-dark-surface-warning-minor': '#3D1D0A',
        '--on-dark-surface-warning-minor-hover': '#58290EFF',
        '--on-dark-surface-warning-minor-active': '#2C1507FF',
        '--on-dark-surface-negative-minor': '#4A0D13',
        '--on-dark-surface-negative-minor-hover': '#64121AFF',
        '--on-dark-surface-negative-minor-active': '#380A0FFF',
        '--on-dark-surface-info-minor': '#0C283B',
        '--on-dark-surface-info-minor-hover': '#10344CFF',
        '--on-dark-surface-info-minor-active': '#091D2AFF',
        '--on-dark-surface-transparent-positive': '#1A9E3233',
        '--on-dark-surface-transparent-positive-hover': '#1A9E3242',
        '--on-dark-surface-transparent-positive-active': '#1A9E3247',
        '--on-dark-surface-transparent-warning': '#FA5F0533',
        '--on-dark-surface-transparent-warning-hover': '#FA5F0542',
        '--on-dark-surface-transparent-warning-active': '#FA5F0547',
        '--on-dark-surface-transparent-negative': '#FF293E33',
        '--on-dark-surface-transparent-negative-hover': '#FF293E42',
        '--on-dark-surface-transparent-negative-active': '#FF293E47',
        '--on-dark-surface-transparent-info': '#118CDF33',
        '--on-dark-surface-transparent-info-hover': '#118CDF42',
        '--on-dark-surface-transparent-info-active': '#118CDF47',
        '--on-dark-surface-accent-gradient': 'linear-gradient(45deg, #15B015 0%, #04C6C9 100%)',
        '--on-light-surface-solid-primary': '#F2F5F8',
        '--on-light-surface-solid-primary-hover': '#EEF2F6FF',
        '--on-light-surface-solid-primary-active': '#EBF0F4FF',
        '--on-light-surface-solid-primary-brightness': '#FFFFFFFF',
        '--on-light-surface-solid-secondary': '#E8EEF2',
        '--on-light-surface-solid-secondary-hover': '#E2E9EEFF',
        '--on-light-surface-solid-secondary-active': '#DEE7EDFF',
        '--on-light-surface-solid-tertiary': '#D5DFE6',
        '--on-light-surface-solid-tertiary-hover': '#CFDAE2FF',
        '--on-light-surface-solid-tertiary-active': '#C9D5DEFF',
        '--on-light-surface-solid-card': '#FFFFFF',
        '--on-light-surface-solid-card-hover': '#FFFFFFFF',
        '--on-light-surface-solid-card-active': '#FFFFFFFF',
        '--on-light-surface-solid-card-brightness': '#FFFFFFFF',
        '--on-light-surface-solid-default': '#060A0C',
        '--on-light-surface-solid-default-hover': '#111C22FF',
        '--on-light-surface-solid-default-active': '#030607FF',
        '--on-light-surface-transparent-primary-hover': '#070B0E0A',
        '--on-light-surface-transparent-primary-active': '#070B0E0D',
        '--on-light-surface-transparent-secondary-hover': '#070B0E14',
        '--on-light-surface-transparent-secondary-active': '#070B0E1A',
        '--on-light-surface-transparent-tertiary': '#060A0C1F',
        '--on-light-surface-transparent-tertiary-hover': '#070B0E2E',
        '--on-light-surface-transparent-tertiary-active': '#070B0E33',
        '--on-light-surface-transparent-deep': '#060A0CA3',
        '--on-light-surface-transparent-deep-hover': '#070B0EB3',
        '--on-light-surface-transparent-deep-active': '#070B0EB8',
        '--on-light-surface-transparent-card': 'rgba(255, 255, 255, 0.6)',
        '--on-light-surface-transparent-card-hover': '#FFFFFFA8',
        '--on-light-surface-transparent-card-active': '#FFFFFFAD',
        '--on-light-surface-transparent-card-brightness': '#FFFFFF99',
        '--on-light-surface-clear': '#FFFFFF00',
        '--on-light-surface-clear-hover': '#FFFFFFFF',
        '--on-light-surface-clear-active': '#FFFFFFFF',
        '--on-light-surface-accent': '#1A9E32',
        '--on-light-surface-accent-hover': '#1EB83AFF',
        '--on-light-surface-accent-active': '#178C2CFF',
        '--on-light-surface-accent-gradient-hover': '#FFFFFFFF',
        '--on-light-surface-accent-gradient-active': '#FFFFFFFF',
        '--on-light-surface-accent-minor': '#9EFAAF',
        '--on-light-surface-accent-minor-hover': '#94F9A7FF',
        '--on-light-surface-accent-minor-active': '#8BF99FFF',
        '--on-light-surface-transparent-accent': '#1A9E321F',
        '--on-light-surface-transparent-accent-hover': '#1A9E322E',
        '--on-light-surface-transparent-accent-active': '#1A9E3233',
        '--on-light-surface-positive': '#1A9E32',
        '--on-light-surface-positive-hover': '#1EB83AFF',
        '--on-light-surface-positive-active': '#178C2CFF',
        '--on-light-surface-warning': '#FA5F05',
        '--on-light-surface-warning-hover': '#FB782DFF',
        '--on-light-surface-warning-active': '#E65705FF',
        '--on-light-surface-negative': '#FF293E',
        '--on-light-surface-negative-hover': '#FF5263FF',
        '--on-light-surface-negative-active': '#FF142CFF',
        '--on-light-surface-info': '#118CDF',
        '--on-light-surface-info-hover': '#209CEEFF',
        '--on-light-surface-info-active': '#0F81CCFF',
        '--on-light-surface-positive-minor': '#9EFAAF',
        '--on-light-surface-positive-minor-hover': '#94F9A7FF',
        '--on-light-surface-positive-minor-active': '#8BF99FFF',
        '--on-light-surface-warning-minor': '#FEE2D2',
        '--on-light-surface-warning-minor-hover': '#FEDCC8FF',
        '--on-light-surface-warning-minor-active': '#FED9C3FF',
        '--on-light-surface-negative-minor': '#FFE0E3',
        '--on-light-surface-negative-minor-hover': '#FFD6DAFF',
        '--on-light-surface-negative-minor-active': '#FFD1D6FF',
        '--on-light-surface-info-minor': '#CFECFF',
        '--on-light-surface-info-minor-hover': '#C7E9FFFF',
        '--on-light-surface-info-minor-active': '#C2E7FFFF',
        '--on-light-surface-transparent-positive': '#1A9E321F',
        '--on-light-surface-transparent-positive-hover': '#1A9E322E',
        '--on-light-surface-transparent-positive-active': '#1A9E3233',
        '--on-light-surface-transparent-warning': '#FA5F051F',
        '--on-light-surface-transparent-warning-hover': '#FA5F052E',
        '--on-light-surface-transparent-warning-active': '#FA5F0533',
        '--on-light-surface-transparent-negative': '#FF293E1F',
        '--on-light-surface-transparent-negative-hover': '#FF293E2E',
        '--on-light-surface-transparent-negative-active': '#FF293E33',
        '--on-light-surface-transparent-info': '#118CDF1F',
        '--on-light-surface-transparent-info-hover': '#118CDF2E',
        '--on-light-surface-transparent-info-active': '#118CDF33',
        '--on-light-surface-transparent-primary': 'rgba(6,10,12,0.03)',
        '--on-light-surface-transparent-secondary': 'rgba(6,10,12,0.06)',
        '--on-light-surface-accent-gradient': 'linear-gradient(45deg, #15B015 0%, #04C6C9 100%)',
        '--inverse-surface-solid-primary': '#13181B',
        '--inverse-surface-solid-primary-hover': '#20282DFF',
        '--inverse-surface-solid-primary-active': '#0B0D0FFF',
        '--inverse-surface-solid-primary-brightness': '#20282DFF',
        '--inverse-surface-solid-secondary': '#23292D',
        '--inverse-surface-solid-secondary-hover': '#2C343AFF',
        '--inverse-surface-solid-secondary-active': '#1B1F23FF',
        '--inverse-surface-solid-tertiary': '#30373C',
        '--inverse-surface-solid-tertiary-hover': '#394147FF',
        '--inverse-surface-solid-tertiary-active': '#272C30FF',
        '--inverse-surface-solid-card': '#13181B',
        '--inverse-surface-solid-card-hover': '#20282DFF',
        '--inverse-surface-solid-card-active': '#0B0D0FFF',
        '--inverse-surface-solid-card-brightness': '#20282DFF',
        '--inverse-surface-solid-default': '#F7F9FB',
        '--inverse-surface-solid-default-hover': '#F5F7FAFF',
        '--inverse-surface-solid-default-active': '#F1F5F8FF',
        '--inverse-surface-transparent-primary': 'rgba(255, 255, 255, 0.06)',
        '--inverse-surface-transparent-primary-hover': '#FFFFFF14',
        '--inverse-surface-transparent-primary-active': '#FFFFFF1A',
        '--inverse-surface-transparent-secondary': 'rgba(255, 255, 255, 0.12)',
        '--inverse-surface-transparent-secondary-hover': '#FFFFFF2E',
        '--inverse-surface-transparent-secondary-active': '#FFFFFF33',
        '--inverse-surface-transparent-tertiary': 'rgba(255, 255, 255, 0.2)',
        '--inverse-surface-transparent-tertiary-hover': '#FFFFFF42',
        '--inverse-surface-transparent-tertiary-active': '#FFFFFF47',
        '--inverse-surface-transparent-deep': 'rgba(255, 255, 255, 0.64)',
        '--inverse-surface-transparent-deep-hover': '#FFFFFFB3',
        '--inverse-surface-transparent-deep-active': '#FFFFFFB8',
        '--inverse-surface-transparent-card': 'rgba(255, 255, 255, 0.06)',
        '--inverse-surface-transparent-card-hover': '#FFFFFF14',
        '--inverse-surface-transparent-card-active': '#FFFFFF1A',
        '--inverse-surface-transparent-card-brightness': '#FFFFFF0F',
        '--inverse-surface-clear': '#FFFFFF00',
        '--inverse-surface-clear-hover': '#FFFFFFFF',
        '--inverse-surface-clear-active': '#FFFFFFFF',
        '--inverse-surface-accent': '#1A9E32',
        '--inverse-surface-accent-hover': '#1EB83AFF',
        '--inverse-surface-accent-active': '#178C2CFF',
        '--inverse-surface-accent-gradient-hover': '#FFFFFFFF',
        '--inverse-surface-accent-gradient-active': '#FFFFFFFF',
        '--inverse-surface-accent-minor': '#0A2B10',
        '--inverse-surface-accent-minor-hover': '#0E3A16FF',
        '--inverse-surface-accent-minor-active': '#061909FF',
        '--inverse-surface-transparent-accent': '#1A9E3233',
        '--inverse-surface-transparent-accent-hover': '#1A9E3242',
        '--inverse-surface-transparent-accent-active': '#1A9E3247',
        '--inverse-surface-positive': '#1A9E32',
        '--inverse-surface-positive-hover': '#1EB83AFF',
        '--inverse-surface-positive-active': '#178C2CFF',
        '--inverse-surface-warning': '#FA5F05',
        '--inverse-surface-warning-hover': '#FB782DFF',
        '--inverse-surface-warning-active': '#E65705FF',
        '--inverse-surface-negative': '#FF293E',
        '--inverse-surface-negative-hover': '#FF5263FF',
        '--inverse-surface-negative-active': '#FF142CFF',
        '--inverse-surface-info': '#118CDF',
        '--inverse-surface-info-hover': '#209CEEFF',
        '--inverse-surface-info-active': '#0F81CCFF',
        '--inverse-surface-positive-minor': '#0A2B10',
        '--inverse-surface-positive-minor-hover': '#0E3A16FF',
        '--inverse-surface-positive-minor-active': '#061909FF',
        '--inverse-surface-warning-minor': '#3D1D0A',
        '--inverse-surface-warning-minor-hover': '#58290EFF',
        '--inverse-surface-warning-minor-active': '#2C1507FF',
        '--inverse-surface-negative-minor': '#4A0D13',
        '--inverse-surface-negative-minor-hover': '#64121AFF',
        '--inverse-surface-negative-minor-active': '#380A0FFF',
        '--inverse-surface-info-minor': '#0C283B',
        '--inverse-surface-info-minor-hover': '#10344CFF',
        '--inverse-surface-info-minor-active': '#091D2AFF',
        '--inverse-surface-transparent-positive': '#1A9E3233',
        '--inverse-surface-transparent-positive-hover': '#1A9E3242',
        '--inverse-surface-transparent-positive-active': '#1A9E3247',
        '--inverse-surface-transparent-warning': '#FA5F0533',
        '--inverse-surface-transparent-warning-hover': '#FA5F0542',
        '--inverse-surface-transparent-warning-active': '#FA5F0547',
        '--inverse-surface-transparent-negative': '#FF293E33',
        '--inverse-surface-transparent-negative-hover': '#FF293E42',
        '--inverse-surface-transparent-negative-active': '#FF293E47',
        '--inverse-surface-transparent-info': '#118CDF33',
        '--inverse-surface-transparent-info-hover': '#118CDF42',
        '--inverse-surface-transparent-info-active': '#118CDF47',
        '--inverse-surface-accent-gradient': 'linear-gradient(45deg, #15B015 0%, #04C6C9 100%)',
        '--background-primary': '#F7F9FB',
        '--dark-background-primary': '#060A0C',
        '--light-background-primary': '#F7F9FB',
        '--inverse-background-primary': '#060A0C',
        '--overlay-soft': '#F7F9FB8F',
        '--overlay-hard': '#F7F9FBF5',
        '--overlay-blur': '#F7F9FB47',
        '--on-dark-overlay-soft': '#060A0C8F',
        '--on-dark-overlay-hard': '#060A0CF5',
        '--on-dark-overlay-blur': '#060A0C47',
        '--on-light-overlay-soft': '#F7F9FB8F',
        '--on-light-overlay-hard': '#F7F9FBF5',
        '--on-light-overlay-blur': '#F7F9FB47',
        '--inverse-overlay-soft': '#060A0C8F',
        '--inverse-overlay-hard': '#060A0CF5',
        '--inverse-overlay-blur': '#060A0C47',
        '--outline-solid-primary': '#D5DFE6',
        '--outline-solid-primary-hover': '#96AEC0FF',
        '--outline-solid-primary-active': '#B6C7D3FF',
        '--outline-solid-secondary': '#ADB9C2',
        '--outline-solid-secondary-hover': '#DCE1E5FF',
        '--outline-solid-secondary-active': '#8496A4FF',
        '--outline-solid-tertiary': '#657179',
        '--outline-solid-tertiary-hover': '#9BA5ABFF',
        '--outline-solid-tertiary-active': '#545D64FF',
        '--outline-solid-default': '#060A0C',
        '--outline-solid-default-hover': '#304F5FFF',
        '--outline-solid-default-active': '#1B2D36FF',
        '--outline-transparent-primary': '#060A0C1F',
        '--outline-transparent-primary-hover': '#070B0E52',
        '--outline-transparent-primary-active': '#070B0E38',
        '--outline-transparent-secondary': '#060A0C47',
        '--outline-transparent-secondary-hover': '#070B0E2B',
        '--outline-transparent-secondary-active': '#070B0E72',
        '--outline-transparent-tertiary': '#060A0C8F',
        '--outline-transparent-tertiary-hover': '#070B0E47',
        '--outline-transparent-tertiary-active': '#070B0EFF',
        '--outline-clear': '#FFFFFF00',
        '--outline-clear-hover': '#FFFFFFFF',
        '--outline-clear-active': '#FFFFFFFF',
        '--outline-accent': '#108E26',
        '--outline-accent-hover': '#15BC31FF',
        '--outline-accent-active': '#0C6A1BFF',
        '--outline-accent-minor': '#28D247',
        '--outline-accent-minor-hover': '#7AE68EFF',
        '--outline-accent-minor-active': '#23B83EFF',
        '--outline-transparent-accent': '#108E2633',
        '--outline-transparent-accent-hover': '#108E251F',
        '--outline-transparent-accent-active': '#108E2552',
        '--outline-positive': '#108E26',
        '--outline-positive-hover': '#15BC31FF',
        '--outline-positive-active': '#0C6A1BFF',
        '--outline-warning': '#E85702',
        '--outline-warning-hover': '#FD9253FF',
        '--outline-warning-active': '#CA4B02FF',
        '--outline-negative': '#F31B31',
        '--outline-negative-hover': '#F87C89FF',
        '--outline-negative-active': '#E40C22FF',
        '--outline-info': '#0B7ECB',
        '--outline-info-hover': '#47B0F5FF',
        '--outline-info-active': '#096CAEFF',
        '--outline-positive-minor': '#28D247',
        '--outline-positive-minor-hover': '#7AE68EFF',
        '--outline-positive-minor-active': '#23B83EFF',
        '--outline-warning-minor': '#FD9C68',
        '--outline-warning-minor-hover': '#FDB790FF',
        '--outline-warning-minor-active': '#FC8240FF',
        '--outline-negative-minor': '#FF8F9A',
        '--outline-negative-minor-hover': '#FFB8BFFF',
        '--outline-negative-minor-active': '#FF6675FF',
        '--outline-info-minor': '#52BAFF',
        '--outline-info-minor-hover': '#A3DAFFFF',
        '--outline-info-minor-active': '#29A9FFFF',
        '--outline-transparent-positive': '#108E2633',
        '--outline-transparent-positive-hover': '#108E251F',
        '--outline-transparent-positive-active': '#108E2552',
        '--outline-transparent-warning': '#E8570233',
        '--outline-transparent-warning-hover': '#E857021F',
        '--outline-transparent-warning-active': '#E8570252',
        '--outline-transparent-negative': '#F31B3133',
        '--outline-transparent-negative-hover': '#F31B311F',
        '--outline-transparent-negative-active': '#F31B3152',
        '--outline-transparent-info': '#0B7ECB33',
        '--outline-transparent-info-hover': '#0B7ECB1F',
        '--outline-transparent-info-active': '#0B7ECB52',
        '--on-dark-outline-solid-primary': '#23292D',
        '--on-dark-outline-solid-primary-hover': '#97A6AFFF',
        '--on-dark-outline-solid-primary-active': '#505E68FF',
        '--on-dark-outline-solid-secondary': '#485056',
        '--on-dark-outline-solid-secondary-hover': '#98A2A9FF',
        '--on-dark-outline-solid-secondary-active': '#434B51FF',
        '--on-dark-outline-solid-tertiary': '#7B868E',
        '--on-dark-outline-solid-tertiary-hover': '#B2B9BDFF',
        '--on-dark-outline-solid-tertiary-active': '#677279FF',
        '--on-dark-outline-solid-default': '#F7F9FB',
        '--on-dark-outline-solid-default-hover': '#E4EBF1FF',
        '--on-dark-outline-solid-default-active': '#D6E0EAFF',
        '--on-dark-outline-transparent-primary-hover': '#F2F5F852',
        '--on-dark-outline-transparent-primary-active': '#F2F5F838',
        '--on-dark-outline-transparent-secondary-hover': '#F2F5F82B',
        '--on-dark-outline-transparent-secondary-active': '#F2F5F872',
        '--on-dark-outline-transparent-tertiary-hover': '#F2F5F847',
        '--on-dark-outline-transparent-tertiary-active': '#F2F5F8FF',
        '--on-dark-outline-clear': '#FFFFFF00',
        '--on-dark-outline-clear-hover': '#FFFFFFFF',
        '--on-dark-outline-clear-active': '#FFFFFFFF',
        '--on-dark-outline-accent': '#24B23E',
        '--on-dark-outline-accent-hover': '#5EDE76FF',
        '--on-dark-outline-accent-active': '#1F9835FF',
        '--on-dark-outline-accent-minor': '#095C18',
        '--on-dark-outline-accent-minor-hover': '#0F9527FF',
        '--on-dark-outline-accent-minor-active': '#074A14FF',
        '--on-dark-outline-transparent-accent': '#24B23E47',
        '--on-dark-outline-transparent-accent-hover': '#24B23E2B',
        '--on-dark-outline-transparent-accent-active': '#24B23E72',
        '--on-dark-outline-positive': '#24B23E',
        '--on-dark-outline-positive-hover': '#5EDE76FF',
        '--on-dark-outline-positive-active': '#1F9835FF',
        '--on-dark-outline-warning': '#FF7024',
        '--on-dark-outline-warning-hover': '#FFA575FF',
        '--on-dark-outline-warning-active': '#FA5700FF',
        '--on-dark-outline-negative': '#FF3D51',
        '--on-dark-outline-negative-hover': '#FF8F9AFF',
        '--on-dark-outline-negative-active': '#FF142CFF',
        '--on-dark-outline-info': '#199AF0',
        '--on-dark-outline-info-hover': '#79C4F6FF',
        '--on-dark-outline-info-active': '#0E8ADDFF',
        '--on-dark-outline-positive-minor': '#095C18',
        '--on-dark-outline-positive-minor-hover': '#0F9527FF',
        '--on-dark-outline-positive-minor-active': '#074A14FF',
        '--on-dark-outline-warning-minor': '#85380C',
        '--on-dark-outline-warning-minor-hover': '#B14B10FF',
        '--on-dark-outline-warning-minor-active': '#5D2809FF',
        '--on-dark-outline-negative-minor': '#9C1422',
        '--on-dark-outline-negative-minor-hover': '#DD1D30FF',
        '--on-dark-outline-negative-minor-active': '#8C121EFF',
        '--on-dark-outline-info-minor': '#0D5382',
        '--on-dark-outline-info-minor-hover': '#137DC3FF',
        '--on-dark-outline-info-minor-active': '#093B5DFF',
        '--on-dark-outline-transparent-positive': '#24B23E47',
        '--on-dark-outline-transparent-positive-hover': '#24B23E2B',
        '--on-dark-outline-transparent-positive-active': '#24B23E72',
        '--on-dark-outline-transparent-warning': '#FF702447',
        '--on-dark-outline-transparent-warning-hover': '#FF70242B',
        '--on-dark-outline-transparent-warning-active': '#FF702472',
        '--on-dark-outline-transparent-negative': '#FF3D5147',
        '--on-dark-outline-transparent-negative-hover': '#FF3D512B',
        '--on-dark-outline-transparent-negative-active': '#FF3D5172',
        '--on-dark-outline-transparent-info': '#199AF047',
        '--on-dark-outline-transparent-info-hover': '#199AF02B',
        '--on-dark-outline-transparent-info-active': '#199AF072',
        '--on-dark-outline-transparent-primary': 'rgba(242,245,248,0.12)',
        '--on-dark-outline-transparent-secondary': 'rgba(242,245,248,0.28)',
        '--on-dark-outline-transparent-tertiary': 'rgba(242,245,248,0.56)',
        '--on-light-outline-solid-primary': '#D5DFE6',
        '--on-light-outline-solid-primary-hover': '#96AEC0FF',
        '--on-light-outline-solid-primary-active': '#B6C7D3FF',
        '--on-light-outline-solid-secondary': '#ADB9C2',
        '--on-light-outline-solid-secondary-hover': '#DCE1E5FF',
        '--on-light-outline-solid-secondary-active': '#8496A4FF',
        '--on-light-outline-solid-tertiary': '#657179',
        '--on-light-outline-solid-tertiary-hover': '#9BA5ABFF',
        '--on-light-outline-solid-tertiary-active': '#545D64FF',
        '--on-light-outline-solid-default': '#060A0C',
        '--on-light-outline-solid-default-hover': '#304F5FFF',
        '--on-light-outline-solid-default-active': '#1B2D36FF',
        '--on-light-outline-transparent-primary': '#060A0C1F',
        '--on-light-outline-transparent-primary-hover': '#070B0E52',
        '--on-light-outline-transparent-primary-active': '#070B0E38',
        '--on-light-outline-transparent-secondary': '#060A0C47',
        '--on-light-outline-transparent-secondary-hover': '#070B0E2B',
        '--on-light-outline-transparent-secondary-active': '#070B0E72',
        '--on-light-outline-transparent-tertiary': '#060A0C8F',
        '--on-light-outline-transparent-tertiary-hover': '#070B0E47',
        '--on-light-outline-transparent-tertiary-active': '#070B0EFF',
        '--on-light-outline-clear': '#FFFFFF00',
        '--on-light-outline-clear-hover': '#FFFFFFFF',
        '--on-light-outline-clear-active': '#FFFFFFFF',
        '--on-light-outline-accent': '#108E26',
        '--on-light-outline-accent-hover': '#15BC31FF',
        '--on-light-outline-accent-active': '#0C6A1BFF',
        '--on-light-outline-accent-minor': '#28D247',
        '--on-light-outline-accent-minor-hover': '#7AE68EFF',
        '--on-light-outline-accent-minor-active': '#23B83EFF',
        '--on-light-outline-transparent-accent': '#108E2633',
        '--on-light-outline-transparent-accent-hover': '#108E251F',
        '--on-light-outline-transparent-accent-active': '#108E2552',
        '--on-light-outline-positive': '#108E26',
        '--on-light-outline-positive-hover': '#15BC31FF',
        '--on-light-outline-positive-active': '#0C6A1BFF',
        '--on-light-outline-warning': '#E85702',
        '--on-light-outline-warning-hover': '#FD9253FF',
        '--on-light-outline-warning-active': '#CA4B02FF',
        '--on-light-outline-negative': '#F31B31',
        '--on-light-outline-negative-hover': '#F87C89FF',
        '--on-light-outline-negative-active': '#E40C22FF',
        '--on-light-outline-info': '#0B7ECB',
        '--on-light-outline-info-hover': '#47B0F5FF',
        '--on-light-outline-info-active': '#096CAEFF',
        '--on-light-outline-positive-minor': '#28D247',
        '--on-light-outline-positive-minor-hover': '#7AE68EFF',
        '--on-light-outline-positive-minor-active': '#23B83EFF',
        '--on-light-outline-warning-minor': '#FD9C68',
        '--on-light-outline-warning-minor-hover': '#FDB790FF',
        '--on-light-outline-warning-minor-active': '#FC8240FF',
        '--on-light-outline-negative-minor': '#FF8F9A',
        '--on-light-outline-negative-minor-hover': '#FFB8BFFF',
        '--on-light-outline-negative-minor-active': '#FF6675FF',
        '--on-light-outline-info-minor': '#52BAFF',
        '--on-light-outline-info-minor-hover': '#A3DAFFFF',
        '--on-light-outline-info-minor-active': '#29A9FFFF',
        '--on-light-outline-transparent-positive': '#108E2633',
        '--on-light-outline-transparent-positive-hover': '#108E251F',
        '--on-light-outline-transparent-positive-active': '#108E2552',
        '--on-light-outline-transparent-warning': '#E8570233',
        '--on-light-outline-transparent-warning-hover': '#E857021F',
        '--on-light-outline-transparent-warning-active': '#E8570252',
        '--on-light-outline-transparent-negative': '#F31B3133',
        '--on-light-outline-transparent-negative-hover': '#F31B311F',
        '--on-light-outline-transparent-negative-active': '#F31B3152',
        '--on-light-outline-transparent-info': '#0B7ECB33',
        '--on-light-outline-transparent-info-hover': '#0B7ECB1F',
        '--on-light-outline-transparent-info-active': '#0B7ECB52',
        '--inverse-outline-solid-primary': '#23292D',
        '--inverse-outline-solid-primary-hover': '#97A6AFFF',
        '--inverse-outline-solid-primary-active': '#505E68FF',
        '--inverse-outline-solid-secondary': '#485056',
        '--inverse-outline-solid-secondary-hover': '#98A2A9FF',
        '--inverse-outline-solid-secondary-active': '#434B51FF',
        '--inverse-outline-solid-tertiary': '#7B868E',
        '--inverse-outline-solid-tertiary-hover': '#B2B9BDFF',
        '--inverse-outline-solid-tertiary-active': '#677279FF',
        '--inverse-outline-solid-default': '#F7F9FB',
        '--inverse-outline-solid-default-hover': '#E4EBF1FF',
        '--inverse-outline-solid-default-active': '#D6E0EAFF',
        '--inverse-outline-transparent-primary-hover': '#F2F5F852',
        '--inverse-outline-transparent-primary-active': '#F2F5F838',
        '--inverse-outline-transparent-secondary-hover': '#F2F5F82B',
        '--inverse-outline-transparent-secondary-active': '#F2F5F872',
        '--inverse-outline-transparent-tertiary-hover': '#F2F5F847',
        '--inverse-outline-transparent-tertiary-active': '#F2F5F8FF',
        '--inverse-outline-clear': '#FFFFFF00',
        '--inverse-outline-clear-hover': '#FFFFFFFF',
        '--inverse-outline-clear-active': '#FFFFFFFF',
        '--inverse-outline-accent': '#24B23E',
        '--inverse-outline-accent-hover': '#5EDE76FF',
        '--inverse-outline-accent-active': '#1F9835FF',
        '--inverse-outline-accent-minor': '#095C18',
        '--inverse-outline-accent-minor-hover': '#0F9527FF',
        '--inverse-outline-accent-minor-active': '#074A14FF',
        '--inverse-outline-transparent-accent': '#24B23E47',
        '--inverse-outline-transparent-accent-hover': '#24B23E2B',
        '--inverse-outline-transparent-accent-active': '#24B23E72',
        '--inverse-outline-positive': '#24B23E',
        '--inverse-outline-positive-hover': '#5EDE76FF',
        '--inverse-outline-positive-active': '#1F9835FF',
        '--inverse-outline-warning': '#FF7024',
        '--inverse-outline-warning-hover': '#FFA575FF',
        '--inverse-outline-warning-active': '#FA5700FF',
        '--inverse-outline-negative': '#FF3D51',
        '--inverse-outline-negative-hover': '#FF8F9AFF',
        '--inverse-outline-negative-active': '#FF142CFF',
        '--inverse-outline-info': '#199AF0',
        '--inverse-outline-info-hover': '#79C4F6FF',
        '--inverse-outline-info-active': '#0E8ADDFF',
        '--inverse-outline-positive-minor': '#095C18',
        '--inverse-outline-positive-minor-hover': '#0F9527FF',
        '--inverse-outline-positive-minor-active': '#074A14FF',
        '--inverse-outline-warning-minor': '#85380C',
        '--inverse-outline-warning-minor-hover': '#B14B10FF',
        '--inverse-outline-warning-minor-active': '#5D2809FF',
        '--inverse-outline-negative-minor': '#9C1422',
        '--inverse-outline-negative-minor-hover': '#DD1D30FF',
        '--inverse-outline-negative-minor-active': '#8C121EFF',
        '--inverse-outline-info-minor': '#0D5382',
        '--inverse-outline-info-minor-hover': '#137DC3FF',
        '--inverse-outline-info-minor-active': '#093B5DFF',
        '--inverse-outline-transparent-positive': '#24B23E47',
        '--inverse-outline-transparent-positive-hover': '#24B23E2B',
        '--inverse-outline-transparent-positive-active': '#24B23E72',
        '--inverse-outline-transparent-warning': '#FF702447',
        '--inverse-outline-transparent-warning-hover': '#FF70242B',
        '--inverse-outline-transparent-warning-active': '#FF702472',
        '--inverse-outline-transparent-negative': '#FF3D5147',
        '--inverse-outline-transparent-negative-hover': '#FF3D512B',
        '--inverse-outline-transparent-negative-active': '#FF3D5172',
        '--inverse-outline-transparent-info': '#199AF047',
        '--inverse-outline-transparent-info-hover': '#199AF02B',
        '--inverse-outline-transparent-info-active': '#199AF072',
        '--inverse-outline-transparent-primary': 'rgba(242,245,248,0.12)',
        '--inverse-outline-transparent-secondary': 'rgba(242,245,248,0.28)',
        '--inverse-outline-transparent-tertiary': 'rgba(242,245,248,0.56)',
        '--data-default-data-yellow': '#F3A912',
        '--data-default-data-yellow-hover': '#F5B638FF',
        '--data-default-data-yellow-active': '#E49C0CFF',
        '--data-default-data-yellow-minor': '#FFD37A',
        '--data-default-data-yellow-minor-hover': '#FFCF70FF',
        '--data-default-data-yellow-minor-active': '#FFCC66FF',
        '--data-default-data-yellow-transparent': '#F3A9128F',
        '--data-default-data-yellow-transparent-hover': '#F3A8129E',
        '--data-default-data-yellow-transparent-active': '#F3A812A3',
        '--data-on-dark-data-yellow': '#F3A912',
        '--data-on-dark-data-yellow-hover': '#F5B638FF',
        '--data-on-dark-data-yellow-active': '#E49C0CFF',
        '--data-on-dark-data-yellow-minor': '#A16B00',
        '--data-on-dark-data-yellow-minor-hover': '#B87A00FF',
        '--data-on-dark-data-yellow-minor-active': '#8F5F00FF',
        '--data-on-dark-data-yellow-transparent': '#F3A9128F',
        '--data-on-dark-data-yellow-transparent-hover': '#F3A8129E',
        '--data-on-dark-data-yellow-transparent-active': '#F3A812A3',
        '--data-on-light-data-yellow': '#F3A912',
        '--data-on-light-data-yellow-hover': '#F5B638FF',
        '--data-on-light-data-yellow-active': '#E49C0CFF',
        '--data-on-light-data-yellow-minor': '#FFD37A',
        '--data-on-light-data-yellow-minor-hover': '#FFCF70FF',
        '--data-on-light-data-yellow-minor-active': '#FFCC66FF',
        '--data-on-light-data-yellow-transparent': '#F3A9128F',
        '--data-on-light-data-yellow-transparent-hover': '#F3A8129E',
        '--data-on-light-data-yellow-transparent-active': '#F3A812A3',
        '--data-inverse-data-yellow': '#F3A912',
        '--data-inverse-data-yellow-hover': '#F5B638FF',
        '--data-inverse-data-yellow-active': '#E49C0CFF',
        '--data-inverse-data-yellow-minor': '#A16B00',
        '--data-inverse-data-yellow-minor-hover': '#B87A00FF',
        '--data-inverse-data-yellow-minor-active': '#8F5F00FF',
        '--data-inverse-data-yellow-transparent': '#F3A9128F',
        '--data-inverse-data-yellow-transparent-hover': '#F3A8129E',
        '--data-inverse-data-yellow-transparent-active': '#F3A812A3',
        '--skeleton-gradient':
            'linear-gradient( 90deg, rgba(8, 8, 8, 0.09) 0%, rgba(8, 8, 8, 0.08) 6.25%, rgba(8, 8, 8, 0.05) 12.5%, rgba(8, 8, 8, 0.01) 25%, rgba(8, 8, 8, 0.05) 37.5%, rgba(8, 8, 8, 0.08) 43.75%, rgba(8, 8, 8, 0.09) 50%, rgba(8, 8, 8, 0.08) 56.25%, rgba(8, 8, 8, 0.05) 62.5%, rgba(8, 8, 8, 0.01) 75%, rgba(8, 8, 8, 0.05) 87.5%, rgba(8, 8, 8, 0.08) 93.75%, rgba(8, 8, 8, 0.09) 100% )',
        '--skeleton-gradient-lighter':
            'linear-gradient( 90deg, rgba(8, 8, 8, 0.36) 0%, rgba(8, 8, 8, 0.32) 6.25%, rgba(8, 8, 8, 0.20) 12.5%, rgba(8, 8, 8, 0.04) 25%, rgba(8, 8, 8, 0.20) 37.5%, rgba(8, 8, 8, 0.32) 43.75%, rgba(8, 8, 8, 0.36) 50%, rgba(8, 8, 8, 0.08) 56.25%, rgba(8, 8, 8, 0.20) 62.5%, rgba(8, 8, 8, 0.04) 75%, rgba(8, 8, 8, 0.20) 87.5%, rgba(8, 8, 8, 0.32) 93.75%, rgba(8, 8, 8, 0.36) 100% )',
        '--plasma-colors-white-primary': 'var(--on-dark-text-primary)',
        '--plasma-colors-white-secondary': 'var(--on-dark-text-secondary)',
        '--plasma-colors-white-tertiary': 'var(--on-dark-text-tertiary)',
        '--plasma-colors-black-primary': 'var(--on-light-text-primary)',
        '--plasma-colors-black-secondary': 'var(--on-light-text-secondary)',
        '--plasma-colors-black-tertiary': 'var(--on-light-text-tertiary)',
        '--plasma-colors-button-black': 'var(--on-light-surface-solid-default)',
        '--plasma-colors-button-black-secondary': 'var(--on-light-surface-transparent-secondary)',
        '--plasma-colors-button-white': 'var(--on-dark-surface-solid-default)',
        '--plasma-colors-button-white-secondary': 'var(--on-dark-surface-transparent-secondary)',
        '--plasma-colors-text': 'var(--text-primary)',
        '--plasma-colors-primary': 'var(--text-primary)',
        '--plasma-colors-secondary': 'var(--text-secondary)',
        '--plasma-colors-tertiary': 'var(--text-tertiary)',
        '--plasma-colors-paragraph': 'var(--text-paragraph)',
        '--plasma-colors-background': 'var(--background-primary)',
        '--plasma-colors-accent': 'var(--text-accent)',
        '--plasma-colors-success': 'var(--text-positive)',
        '--plasma-colors-warning': 'var(--text-warning)',
        '--plasma-colors-critical': 'var(--text-negative)',
        '--plasma-colors-overlay': 'var(--overlay-soft)',
        '--plasma-colors-surface-liquid01': 'var(--surface-transparent-primary)',
        '--plasma-colors-surface-liquid02': 'var(--surface-transparent-secondary)',
        '--plasma-colors-surface-liquid03': 'var(--surface-transparent-tertiary)',
        '--plasma-colors-surface-solid01': 'var(--surface-solid-primary)',
        '--plasma-colors-surface-solid02': 'var(--surface-solid-secondary)',
        '--plasma-colors-surface-solid03': 'var(--surface-solid-tertiary)',
        '--plasma-colors-surface-card': 'var(--surface-transparent-card)',
        '--plasma-colors-button-secondary': 'var(--surface-transparent-secondary)',
        '--plasma-colors-button-accent': 'var(--text-accent)',
        '--plasma-colors-button-success': 'var(--surface-positive)',
        '--plasma-colors-button-warning': 'var(--surface-warning)',
        '--plasma-colors-button-critical': 'var(--surface-negative)',
        color: 'var(--text-primary)',
        'background-color': 'var(--background-primary)',
    },
};
/** @deprecated использовать вместо этого sdds_insol__light */
export const lightSdds_insol = sdds_insol__light;
