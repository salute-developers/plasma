...
import { NumberInput as NumberInputINSOL } from '.';

describe('sdds-insol: NumberInput', () => {
    const NumberInput = getComponent('NumberInput') as typeof NumberInputINSOL;

    const InteractiveNumberInput = ({ value: outerValue, ...rest }: ComponentProps<typeof NumberInputINSOL>) => {
    ...
    it('[PLASMA-T1453] NumberInput: size=l, view=default, shape=cornered, inputBackgroundType=fill, segmentation=default, width=0', () => {
        mount(
            <CypressTestDecorator>
                <InteractiveNumberInput size="l" min={0} max={9} step={1} width={0} isManualInput value={5} />
            </CypressTestDecorator>,
        );
        cy.get('input').type('9');
    ...
    it('[PLASMA-T1454] NumberInput: size=l, view=default, shape=cornered, inputBackgroundType=fill, segmentation=default, width=300, step=2', () => {
        mount(
            <CypressTestDecorator>
                <InteractiveNumberInput size="l" min={0} max={9} step={2} width={300} value={5} />
            </CypressTestDecorator>,
        );
    ...
    it('[PLASMA-T1455] NumberInput: size=l, maxInput, width=188, step=2', () => {
        mount(
            <CypressTestDecorator>
                <InteractiveNumberInput size="l" min={0} max={9} step={2} width={188} isManualInput value={5} />
            </CypressTestDecorator>,
        );
        cy.get('input').click();
