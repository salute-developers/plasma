...
export const StyledSegmentItem = styled.button<{ $isSelected?: boolean }>`
    height: 1.75rem;
    padding: 0.375rem 0.5rem;
    border: 0;
    border-radius: 999rem;
    cursor: pointer;
    color: ${({ $isSelected }) => ($isSelected ? 'var(--text-primary)' : 'var(--text-secondary)')};
    background: ${({ $isSelected }) => ($isSelected ? 'var(--surface-solid-primary)' : 'transparent')};
    ...
    &:hover {
        ...
        background: ${({ $isSelected }) =>
            $isSelected ? 'var(--surface-solid-primary)' : 'var(--surface-transparent-secondary-hover)'};
    }
`;
