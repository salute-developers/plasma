...
export const Gallery: PageComponent<PageStateType, 'gallery', PageParams> = (props) => {
    ...
    // В качестве галереии используется компонент галереии из пакета @salutejs/plasma-temple
...
В данном случае данные экрана будут инициализироваться не эффекте после монтирования компонента, а до первого рендера путем вызова функции `getInitialProps`, которая должна быть объявлена внутри модуля компонента
...
/*
    По сравнению с предыдущим подходом, состояние не может быть null, т.к. данные инициализируем до первого рендера
*/
export interface PageStateType {
