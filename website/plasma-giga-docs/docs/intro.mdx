...
* Непосредственно из темы `@salutejs/plasma-themes/tokens/plasma_giga` - следует использовать, когда необходимо импортировать уникальные токены, которые лежат только в этой теме.
