---
id: checkbox
slug: /components/checkbox
title: Checkbox
description: "Ввод логического значения — флажок (checkbox)."
---

import { PropsTable, Description } from '@site/src/components';

# Checkbox
<Description name="Checkbox" />

:::caution Устаревшие значения для свойства view
Актуальным значением для свойства `view` - является `default`.

Все остальные значения `deprecated` и будут удалены в ближайшее время!
:::

## Использование
Компонент `Checkbox` может содержать лейбл и описание.

```tsx live
import React from 'react';
import { Checkbox } from '@salutejs/plasma-giga';

export function App() {
    return (
        <Checkbox label="Чекбокс" description="Описание" defaultChecked />
    );
}
```

## Иконка выбранного состояния

Свойство `checkIcon` управляет иконкой выбранного состояния. По умолчанию используется значение `check` — стандартная галочка. Значение `cross` включает предустановленную иконку крестика без необходимости передавать её извне. Также в `checkIcon` можно передать произвольную иконку.

```tsx live
import React from 'react';
import { Checkbox } from '@salutejs/plasma-giga';

export function App() {
    return <Checkbox label="Чекбокс с крестиком" defaultChecked checkIcon="cross" />;
}
```

Свойства `description` и `label` могут принимать JSX элементы.

По умолчанию, контент внутри лейбла и описания многострочный.

Для того чтобы стал однострочным, необходимо использовать свойство `singleLine`(по умолчанию `false`).

```tsx live
import React from 'react';
import { Checkbox } from '@salutejs/plasma-giga';

export function App() {
    return (
        <Checkbox
            label={<div>Чекбокс со <a href="/#">ссылкой</a></div>}
            description={<div>Описание чекбокса со <a href="/#">ссылкой</a></div>}
        />
    );
}
```

Состояние `indeterminate` имеет приоритет и не зависит от свойства `checked`

```tsx live
import React from 'react';
import { Checkbox } from '@salutejs/plasma-giga';

export function App() {
    return (
        <Checkbox label="Чекбокс" description="Описание" defaultChecked indeterminate checked={false} />
    );
}
```

## API

<PropsTable name="Checkbox" exclude={['css', 'focused', 'readOnly']} include={['defaultChecked']} />
