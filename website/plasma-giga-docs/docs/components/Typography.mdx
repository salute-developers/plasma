...
Проп `typograph` есть у текстовых компонентов `Text*`. По нему строковые `children` обрабатываются некоторыми правилами типографики:
...
import { TextM } from '@salutejs/plasma-giga';

export function App() {
    return (
        <TextM typograph style={{ maxWidth: 240 }}>
...
`typograph={true}` (или просто `typograph`) берёт правила из модульного реестра: базовый набор плюс то, что добавили через `addTypographRule`. Менять реестр нужно один раз на старте приложения — уже смонтированные инстансы сами не пересчитаются.

Массив на пропе — полный пайплайн только этого инстанса: реестр не читается, используются переданные функции.
...
После этого любой `Text*` с `typograph` подхватит новое правило:

```tsx
<TextM typograph>в лесу — "цитата" (c)</TextM>
```
...
Локальные правила для отдельного компонента:

```tsx
import { TextM, createTypograph, quotes, dash } from '@salutejs/plasma-giga';

const compact = createTypograph([quotes, dash]);

<TextM typograph={[compact]}>без предлогов</TextM>
```
