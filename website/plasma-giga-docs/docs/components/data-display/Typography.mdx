...
Если строка режется на токены (стрим, markdown), `typograph()` каждый раз начинает кавычки с нуля. Прокидывайте глубину через `applyQuotes`:
...
for (const token of tokens) {
    const next = applyQuotes(token, depth);
    depth = next.depth;
    // next.text
}
```

`quotes(text, depth?)` по-прежнему возвращает строку. `typograph()` рассчитан на целую строку и depth не прокидывает.
