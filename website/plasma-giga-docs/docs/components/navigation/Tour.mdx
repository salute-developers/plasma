...
import { Tour, Button } from '@salutejs/plasma-giga';
...
```tsx live
import React, { useRef, useState } from 'react';
import { Tour, Button, TextField, Switch } from '@salutejs/plasma-giga';
...
```tsx live
import React, { useRef, useState } from 'react';
import { Tour, TourCard, Button, ButtonGroup, LinkButton } from '@salutejs/plasma-giga';
...
export function App() {
    ...
    const steps: Step[] = [
        { target: ref1, title: 'Шаг 1', description: 'Карточка со стрелкой.' },
        { target: ref2, title: 'Шаг 2', description: 'Стрелка меняет направление автоматически.' },
    ];

    const renderTourCard = (stepCurrent: number, stepLength: number, last: boolean, step: Step) => {
        const actionButtons = (
        ...
        const skipButton = <LinkButton onClick={onClose}>Пропустить</LinkButton>;
    ...
    return (
        <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '4rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Button ref={ref1} view="secondary">
                    Кнопка 1
                </Button>
            ...
            <Button view="default" onClick={() => setOpen(true)}>
                Запустить тур
            </Button>
