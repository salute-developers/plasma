...
import { ProgressBarCircular } from '@salutejs/sdds-crm';
...
export function App() {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <ProgressBarCircular value={50} strokeWidth={2} />
            <ProgressBarCircular value={50} strokeWidth={4} />
            <ProgressBarCircular value={50} strokeWidth={8} />
            <ProgressBarCircular value={50} strokeWidth={12} />
        </div>
    );
}
```
