---
id: tabs
title: Tabs
---

import { PropsTable, Description, StorybookLink } from '@site/src/components';

# Tabs
Набор компонентов для создания вкладок.

<StorybookLink name="Tabs" />

```tsx live
import React, { useState } from 'react';
import { Tabs, TabItem, DeviceThemeProvider, Container } from '@salutejs/plasma-ui';
import { IconDownload } from '@salutejs/plasma-icons';

export function App() {
    const items = [{ label: 'Item 1' }, { label: 'Item 2' }, { label: 'Item 3' }, { label: 'Item 4' }];
    const [index, setIndex] = useState(0);

    return (
        <div style={{zIndex: '0', position: 'relative'}}>
            <DeviceThemeProvider>
                <Container>
                    <Tabs
                        size="l"
                        view="secondary"
                        stretch={true}
                        pilled={false}
                        scaleOnPress={true}
                        outlined={false}
                        index={index}
                        animated={true}
                    >
                        {items.map((_, i) => (
                            <TabItem
                                key={`item:${i}`}
                                isActive={i === index}
                                tabIndex={0}
                                contentLeft={<IconDownload size="s" />}
                                onClick={() => setIndex(i)}
                            >
                                Label
                            </TabItem>
                        ))}
                    </Tabs>
                </Container>
            </DeviceThemeProvider>
        </div>
    );
}
```

## Tabs
Основной компонент, контейнер вкладок.

<PropsTable name="Tabs" />

## TabItem
Элемент списка. Нельзя использовать вне компонента `Tabs`.

<PropsTable name="TabItem" />

## Смена тега
По умолчанию компоненты вкладок используют теги `<div>` и `<button>` для `Tabs` и `TabItem` соответственно.
Иногда может понадобиться сменить эти теги на теги маркированных списков `<ul>` и `<li>`.
Чтобы сделать это, используйте свойство `forwardedAs`:

```tsx live
import React from 'react';
import { Tabs, TabItem } from '@salutejs/plasma-ui';

export function App() {
    return (
        <div style={{zIndex: '0', position: 'relative'}}>
            <Tabs forwardedAs="ul">
                <TabItem isActive forwardedAs="li">Каждый</TabItem>
                <TabItem forwardedAs="li">Охотник</TabItem>
                <TabItem forwardedAs="li">Желает</TabItem>
                <TabItem forwardedAs="li">Знать</TabItem>
                <TabItem forwardedAs="li">Где</TabItem>
                <TabItem forwardedAs="li">Сидит</TabItem>
                <TabItem forwardedAs="li">Фазан</TabItem>
            </Tabs>
        </div>
    );
}
```

## Подскрол табов
Если нужно чтобы выбранный таб всегда был виден. Используйте свойство autoscroll

```tsx live
import React, { useState } from 'react';
import { TabsController } from '@salutejs/plasma-ui';

export function App() {
    const items = [{ label: 'Item 1' }, { label: 'Item 2' }, { label: 'Item 3' }, { label: 'Item 4' }];
    const [index, setIndex] = useState(0);

    return (
        <div style={{zIndex: '0', position: 'relative'}}>
            <TabsController items={items} index={index} onIndexChange={(i) => setIndex(i)} autoscroll/>
        </div>
    );
}
```
