...
### Перенос 
По умолчанию для всех компонентов типографики в теме включен перенос текста. Если необходимо отключить данное поведение, нужно указать `breakWard={false}` в провайдере темы `DeviceThemeProvider`.
