...
const lightThemeStyle = css(sdds_insol__light);
const darkThemeStyle = css(sdds_insol__dark);
...
Для корректной работы `SSR - insoler side rendering` приложение нужно обернуть `SSRProvider`;
