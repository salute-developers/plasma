...
Анимацию нажатия можно задать непосредственно для корневого элемента Button. В примере кнопка уменьшается и подсвечивается при клике.
...
export function App() {
    const AnimatedButton = styled(Button)`
        ...
        &.animation-playing::after {
            animation: button-flash 300ms linear;
        }
