...
import { Attach } from '@salutejs/sdds-sbcom';
...
import { Attach, Button, InformationWrapper } from '@salutejs/sdds-sbcom';
...
function App() {
    ...
    return (
        <>
            {!isLoading && (
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '16px 18px', border: `1px dashed gray` }}>
