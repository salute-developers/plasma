...
<Tabs>
    <TabItem value="default" label="Uncontrolled" default>
        ...
        import { TextArea } from '@salutejs/sdds-sbcom';
...
```tsx live
import React from 'react';
import { TextArea, BodyS } from '@salutejs/sdds-sbcom';

export function App() {
    const HeaderSlot = () => (
       <div style={{ margin: "0 0.75rem", padding: "0.5rem 0", borderBottom: `0.063rem solid var(--surface-solid-tertiary)` }}>
           <BodyS>Дополнительный контент</BodyS>
       </div>
    )
...
Для анимации изменения высоты, очистки и плавающего label доступны CSS-селекторы:

- `.textarea` — нативный textarea;
- `.textarea-placeholder` — внешний или плавающий label;
- `.textarea-wrapper` — обёртка textarea.

```tsx live
import React, { useState } from 'react';
import styled from 'styled-components';
import { IconClose } from '@salutejs/plasma-icons';
import { EmbedIconButton, TextArea } from '@salutejs/sdds-sbcom';
...
export function App() {
    const AnimatedTextArea = styled(TextArea)`
        ...
        .inner-placeholder-up .textarea-placeholder {
            opacity: 0.9;
        }
    `;

    function Demo() {
        const [value, setValue] = useState('Первая строка');
