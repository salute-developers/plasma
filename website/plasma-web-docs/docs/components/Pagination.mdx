...
# Pagination
Pagination может отображаться в разных размерах и может содержать селект с выбором количеста элементов на странице и быстрый выбор страницы

## Pagination
<Description name="Pagination" />
<PropsTable name="Pagination" exclude={['css', 'focused']} />
<StorybookLink name="Pagination" />

## Использование
Компонент `Pagination` должен содержать параметр slots. Он обозначает количество кнопок в пагинции
...
export function App() {
    return (
        <div style={{display: "flex", "flex-direction": "column"}}>
            <Pagination size="l" slots={9} count={200} hasPerPage={true} helperText="Helper Text" />
            <Pagination size="m" slots={9} count={200} hasPerPage={true} helperText="Helper Text" />
            <Pagination size="s" slots={9} count={200} hasPerPage={true} helperText="Helper Text" />
            <Pagination size="xs" slots={9} count={200} hasPerPage={true} helperText="Helper Text" />
        </div>
    );
}
```
