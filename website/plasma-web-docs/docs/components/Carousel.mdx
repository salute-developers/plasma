...
export const MyGalleryItem = ({ index, children }) => (
    <CarouselItem scrollSnapAlign="center" index={index}>{children}</CarouselItem>
);
```
...
```tsx live
import React from 'react';
import { Carousel, CarouselItem } from '@salutejs/plasma-web';
...
export function App() {
    ...
    return (
        ...
        >
            {items.map((item, i) => (
                <CarouselItem key={`item:${i}`} index={i} style={{ display: 'flex', alignItems: 'center', padding: '0 0.5rem' }} scrollSnapAlign={align}>
                    <StyledCard>
                        {item.title}
                    </StyledCard>
                </CarouselItem>
...
<section id="carousel-example">
    ...
    <Carousel>
        {items.map((item, i) => <CarouselItem aria-label={`${i + 1} из ${items.length}`} index={i} />)}
    </Carousel>
</section>
```
