---
id: typography-legacy
slug: /components/typography-legacy
title: Typography (legacy)
description: "Устаревшие компоненты типографики, оставленные для обратной совместимости."
---

import { StorybookLink } from '@site/src/components';

:::caution
Текст данной страницы относится к функциональности, которая больше не поддерживается. С актуальной информацией можно ознакомиться на странице [Typography](../typography).
:::
...
## Моноширинные числа
Если необходимо сделать числа моноширинным, то нужно установить свойство `isNumeric` в значении `true`.
...
```tsx live
import React from 'react';
import { Headline1, Headline2, Headline3, Headline4 } from '@salutejs/plasma-web';
...
```tsx live
import React from 'react';
import { Body1, Body2, ParagraphText1, ParagraphText2 } from '@salutejs/plasma-web';
...
```tsx live
import React from 'react';
import { Footnote1, Footnote2 } from '@salutejs/plasma-web';
...
```tsx live
import React from 'react';
import { Button1, Button2 } from '@salutejs/plasma-web';
...
```tsx live
import React from 'react';
import { Caption, Underline } from '@salutejs/plasma-web';
