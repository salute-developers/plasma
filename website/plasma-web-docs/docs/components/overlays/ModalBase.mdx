---
id: modalBase
slug: /components/modalBase
title: ModalBase
description: "Базовый модальный компонент с подложкой, блокирующий скролл и фокус страницы."
---
...
import { Button, ModalBase, PopupBaseProvider } from '@salutejs/plasma-web';
...
Чтобы задать ширину, передайте `style` в `ModalBase`: он попадает на контейнер окна,
...
```tsx
<ModalBase opened focusTrapSelectors={['#fullscreen-table', '.nested-modal']}>
    <Content />
</ModalBase>
```
