...
<Tabs>
    ...
    <TabItem value="dimensions" label="Width and height">
        Выпадающему списку можно задать кастомную ширину и высоту. За это отвечают параметры `"listWidth"` и `"listHeight"`.<br />
        Если задан параметр `"listHeight"`, то в дополнение к нему нужно еще установить `"listOverflow"` для корректного отображения.<br />
        Параметр `"listWidth"` установит указанную ширину для всех вложенных списков.
...
Данный компонент соответствует требования [W3C](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/).
