...
title: Modal
description: "Доступное модальное диалоговое окно."
---
...
import { surfaceSolidSecondary } from '@salutejs/plasma-themes/tokens';
import { Button, Modal, PopupProvider } from '@salutejs/plasma-homeds';
...
Окно не задаёт свои размеры — по умолчанию оно подстраивается под контент.
Чтобы задать ширину, передайте `style` в `Modal`: он попадает на контейнер окна,
поэтому размер работает и со стилизованной обёрткой `hasBody`.
...
import { Modal } from '@salutejs/plasma-homeds';
...
Если часть интерфейса модального окна отрисована через Portal или иным образом находится вне его DOM-поддерева,
передайте селекторы корневых элементов этой части в `focusTrapSelectors`. Совпавшие элементы и их потомки
будут считаться частью модального окна при перемещении фокуса.
...
Чтобы иметь возможность растягивать Modal, нужно настроить свойство `resizable`. Формат следующий:
