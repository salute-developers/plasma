...
id: mixins
title: Миксины
description: "Раздел содержит функции, которые можно применять для стилизации компонент."
---
...
import { addFocus } from '@salutejs/plasma-homeds';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `plasma-homeds`:
...
```tsx live
import { applyPaper } from '@salutejs/plasma-homeds';

<div style={applyPaper({
...
###
```tsx
import { addScrollbar } from '@salutejs/plasma-homeds';
