...
$ npm install --save @salutejs/sdds-insurance @salutejs/sdds-themes
```
...
import { Button, BodyL } from '@salutejs/sdds-insurance';
...
import '@salutejs/sdds-themes/css/sdds_insurance__dark.css';
...
import styles from '@salutejs/sdds-themes/css/sdds_insurance.module.css';
...
Для корректной работы `SSR - insuranceer side rendering` приложение нужно обернуть `SSRProvider`;
...
* Непосредственно из темы `@salutejs/sdds-themes/tokens/sdds_insurance` - следует использовать, когда необходимо импортировать уникальные токены, которые лежат только в этой теме.
