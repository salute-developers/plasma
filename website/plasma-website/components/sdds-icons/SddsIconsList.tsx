...
const StyledGridWrapper = styled.section<{ $hasActiveIcon: boolean }>`
    /* stylelint-disable property-no-unknown */
    content-visibility: ${({ $hasActiveIcon }) => ($hasActiveIcon ? 'visible' : 'auto')};
    ...
    return (
        <StyledIconList>
            {groups.map(({ category, icons: categoryIcons }, indexGroup) => (
                <StyledGridWrapper
                    $hasActiveIcon={categoryIcons.some(({ name }) => name === state.wizardItemName)}
                    key={category}
                >
