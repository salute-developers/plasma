...
import { Footer, Header, IconsToolbar, Main } from '../components/roster';
import { multipleMediaQuery } from '../mixins';
import { StyledActionIcon } from '../components/roster/StyledActionIcon';
import { getFormattedDate } from '../utils';
import { defaultIconStyle, matchesIconStyle } from '../utils/iconStyle';
...
function SddsIconsPage() {
    ...
    const visibleIcons = useMemo(() => {
        ...
        const filteredManifest = {
            ...
            icons: manifestState.manifest.icons.filter(({ name }) => matchesIconStyle(name, iconStyle)),
        };

        if (!searchQuery.trim()) {
            return filteredManifest.icons;
        }

        return searchIcons(filteredManifest, searchQuery);
    }, [iconStyle, manifestState, searchQuery]);

    const manifestVersion = manifestState.status === 'loaded' ? manifestState.manifest.version : undefined;
    const manifestReleaseDate =
        manifestState.status === 'loaded'
            ? getFormattedDate({ date: manifestState.manifest.builtAt }) || undefined
            : undefined;
    ...
    return (
        <ToastProvider>
            ...
            <StyledSection ref={sectionRef}>
                ...
                <Header
                    packageName="@salutejs/sdds-icons"
                    releaseDate={manifestReleaseDate}
                    title="SDDS Icons"
                    version={manifestVersion}
                />
                <StyledMain>
                    <IconsToolbar activeCollection="sdds" iconStyle={iconStyle} onIconStyleChange={setIconStyle} />
                    <StyledSearchSection>
                        <StyledSearchInput
                            ref={inputRef}
