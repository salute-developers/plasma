...
import { addFocus } from '@salutejs/sdds-bizcom';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_bizcom`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-bizcom';
