...
Если передать `contentRight`, компонент полностью отдаёт управление иконкой снаружи и не применяет встроенную анимацию поворота (она работает только для иконки по умолчанию `type="sign"`/`type="arrow"`). Чтобы анимировать кастомную иконку, нужно самостоятельно менять её стиль в зависимости от состояния `opened`.
