...
export function App() {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "32px"}}>
            <Attach view="default" buttonType='button' text="Upload file" />
            <Attach view="default" buttonType='iconButton'/>
        </div>
    );
}
```
        ...
        <>
            {!isLoading && (
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '16px 18px', border: `1px dashed ${outlineSolidPrimary}` }}>
                   ...
                   <Button view="accent" style={{ alignSelf: 'end' }} type="submit">Submit</Button>
                </form>
            )}
