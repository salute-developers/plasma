...
import { addFocus } from '@salutejs/sdds-platform-ai';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_platform_ai`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-platform-ai';
