...
description: "Сервер работает по `stdio` и предоставляет агенту доступ к документации, свойствам компонентов, примерам и гайдам по `sdds-platform-ai`."
slug: /how-to-mcp
sidebar_position: 2
---
Сервер работает по `stdio` и предоставляет агенту доступ к документации, свойствам компонентов, примерам и гайдам по `sdds-platform-ai`.

## Быстрый старт
Если ваш агент умеет подключать MCP-серверы по `stdio`, достаточно указать команду:

```bash
npx -y @salutejs/sdds-mcp@latest --lib sdds-platform-ai --version latest
```
...
Для `sdds-platform-ai` поддерживаются версии, начиная с `0.343.0`.
...
- `Покажи, как подключить sdds-platform-ai в Next.js`
- `Какие props есть у Button в sdds-platform-ai?`
- `Приведи примеры использования TextField`
- `Какие компоненты есть в sdds-platform-ai?`
