...
title: Tour
description: "Пошаговый тур по интерфейсу."
---
...
```tsx live
import React, { useRef, useState } from 'react';
import { Tour, Button, TextField, Switch } from '@salutejs/sdds-cs';
...
Библиотека поставляет компонент `TourCard`, из которого можно собрать карточку шага с изображением, пагинацией и собственными элементами управления.

```tsx live
import React, { useState } from 'react';
import { Avatar, Button, ButtonGroup, Range, Switch, TextField, Tour, TourCard } from '@salutejs/sdds-cs';
...
export function App() {
    ...
    const renderTourCard = (stepCurrent, stepLength, last, step) => {
        const actionButtons = (
            <ButtonGroup stretching="auto" gap="wide" isCommonButtonStyles={false}>
                {stepCurrent > 0 && (
                    <Button size="s" view="secondary" onClick={() => setCurrent((value) => value - 1)}>
                        Назад
                    </Button>
                )}
                ...
                {last ? (
                    ...
                    <Button size="s" view="accent" onClick={() => setCurrent((value) => value + 1)}>
                        Далее
                    </Button>
                )}
            </ButtonGroup>
    ...
    return (
        ...
        >
            <Avatar id="tour-card-avatar" size="m" name="Иван Иванов" style={{ alignSelf: 'flex-start' }} />

            <div id="tour-card-textfield">
                <TextField label="Имя пользователя" value={name} onChange={(event) => setName(event.target.value)} />
            </div>
            ...
            <Range
                ...
                onChangeFirstValue={(event) => setFirstValue(event.target.value)}
                onChangeSecondValue={(event) => setSecondValue(event.target.value)}
            />
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Button id="tour-card-save" view="accent">
                    Сохранить
                </Button>
            </div>
...
В горизонтальной ориентации `contentDirection` меняет взаимное расположение изображения и контента, а `paginationType` позволяет выбрать пагинацию точками (`dot`) или счётчиком (`compact`). Для вертикальной карточки всегда используется пагинация точками.

### Тип компонента TourCard

```tsx
type TourCardProps = {
    /** Ориентация карточки. @default 'horizontal' */
    orientation?: 'horizontal' | 'vertical';
    /** Расположение контента в горизонтальной карточке. @default 'row-reverse' */
    contentDirection?: 'row-reverse' | 'row';
    /** Заголовок карточки. */
    title?: string;
    /** Описание карточки. */
    description?: string;
    /** Свойства изображения: src, alt, ratio, customRatio и другие. */
    image?: ImageProps;
    /** Показывать пагинацию. @default true */
    showPagination?: boolean;
    /** Максимальное количество отображаемых точек пагинации. @default 3 */
    paginationVisibleItems?: number;
    /** Вид пагинации; compact применяется только в горизонтальной карточке. @default 'dot' */
    ...
    /** Показывать кнопку закрытия. @default true */
    showClose?: boolean;
