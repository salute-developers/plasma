...
* Непосредственно из темы `@salutejs/sdds-themes/tokens/sdds_cs` - следует использовать, когда необходимо импортировать уникальные токены, которые лежат только в этой теме.
