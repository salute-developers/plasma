...
import { addFocus } from '@salutejs/sdds-finai';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_finai`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-finai';
