...
title: Range
description: "Ввод диапазона значений «от — до»."
---
...
<Tabs>
    <TabItem value="uncontrolled" label="Uncontrolled" default>
        ...
        import { Range } from '@salutejs/sdds-finai';
...
Внешний вид компонента зависит от свойств `appearance`, `size`. `appearance="viewMode"` — режим просмотра.
...
Размер компонента зависит от свойства `size`:
...
```tsx live
import React from 'react';
import { Range, IconButton } from '@salutejs/sdds-finai';
...
<PropsTable name="Range" exclude={['css', 'appearance', 'hasClearDivider']} />
