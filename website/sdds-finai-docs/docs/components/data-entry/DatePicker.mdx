...
title: DatePicker
description: "Выбор даты или диапазона дат (DatePickerRange)."
---
...
Внешний вид компонента задаётся с помощью свойства `appearance`. `appearance="viewMode"` — режим просмотра.
...
Возможные значения свойства: `"s"`, `"xs"`.
...
<Tabs>
    ...
    <TabItem value="calendar-position" label="isCalendarPositionedByInput">
        Свойство `isCalendarPositionedByInput` привязывает одиночный календарь к активному полю ввода.
        При переключении между полями открытый календарь перемещается под выбранное поле.
        ...
        export function App() {
            return (
                <DatePickerRange
                    size="s"
                    label="Период"
                    firstPlaceholder="Дата начала"
                    secondPlaceholder="Дата окончания"
...
### Независимый выбор дат

Поля начальной и конечной даты заполняются независимо — в любом порядке.

При выборе даты через календарь применяется **умный своп**: если введённые даты расположены в неправильном порядке, они автоматически меняются местами.

При заполнении только второго поля фокус автоматически переходит к первому для завершения диапазона.
...
Свойства `min` и `max` задают допустимый диапазон дат. Свойство `includeEdgeDates` включает граничные даты в допустимый диапазон.
