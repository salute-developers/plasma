---
id: dateTimePicker
slug: /components/dateTimePicker
...
export function App() {
    ...
    return (
        <DateTimePicker
            ...
            label="Квартал и время"
            placeholder="Выберите квартал и время"
            usePortal
        />
    );
}
...
Свойство `hasTimeFormat` включает 12-часовой формат: колонка часов показывает значения `12`, `01`–`11`,
а справа добавляется колонка выбора `AM`/`PM`.

`AM`/`PM` в поле ввода — не часть текста, а слот `textAfter`. Если задать свой `textAfter`,
значения склеиваются: например, `PM TA`.

С клавиатуры `AM`/`PM` не вводится — переключается только колонкой в выпадающем списке.

:::note Значение всегда остаётся 24-часовым
`hasTimeFormat` влияет только на отображение. `timeFormat` по-прежнему задаётся с `HH`, а `value`, `defaultDate`,
`min`, `max` и значение в `onChangeValue` и `onCommitDate` используют 24-часовой формат.
Это позволяет не менять логику приложения при переключении формата.
:::

Полночь и полдень отображаются как `12:00 AM` и `12:00 PM` соответственно.
    ...
    return (
        <div>
            <DateTimePicker
                ...
                placeholder="Введите дату"
                hasTimeFormat
                defaultDate={new Date(2024, 5, 15, 13, 30)}
                maskWithFormat
...
```tsx live
import React from 'react';
import { DateTimePicker, IconButton } from '@salutejs/sdds-finai';
import { IconDone } from '@salutejs/plasma-icons';
