...
Свойство `use12Hours` включает 12-часовой формат: колонка часов показывает значения `00`–`11`,
а справа добавляется колонка выбора `AM`/`PM`.

:::note Значение всегда остаётся 24-часовым
`use12Hours` влияет только на отображение. Свойства `value`, `defaultValue`, `min`, `max`,
`disabledValues` и значение в `onChange` по-прежнему используют 24-часовой формат `HH:mm:ss`.
Это позволяет не менять логику приложения при переключении формата.
:::

Полночь и полдень отображаются как `00:00 AM` и `00:00 PM` соответственно.
