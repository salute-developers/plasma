...
* Непосредственно из темы `@salutejs/sdds-themes/tokens/sdds-finportal` - следует использовать, когда необходимо импортировать уникальные токены, которые используются только в этой теме.
...
```ts
import { BodyL, DsplL, H3 } from '@salutejs/sdds-finportal';
```
:::
