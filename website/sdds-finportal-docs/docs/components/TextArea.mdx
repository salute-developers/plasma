...
import { TextArea } from '@salutejs/sdds-finportal';
...
По умолчанию включены боковые отступы. Их можно отключить с помощью свойства `applyHeaderDefaultPaddings`.
Так же можно выключить отображение divider с помощью `enableHeaderDivider`:

```tsx live
import React from 'react';
import { TextArea, TextS } from '@salutejs/sdds-finportal';

export function App() {
    return (
        <div>
            <TextArea
                headerSlot={<TextS>Дополнительный контент</TextS>}
                enableHeaderDivider
