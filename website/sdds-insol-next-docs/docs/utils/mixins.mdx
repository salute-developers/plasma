...
import { addFocus } from '@salutejs/sdds-insol-next';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_insol`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-insol-next';
