...
const PlasmaTheme = createGlobalStyle(sdds_insol_next__light);
...
```jsx
import React from 'react';
import { Button } from '@salutejs/sdds-insol-next/emotion'
...
const nextConfig = {
  ...
  transpilePackages: ['@salutejs/sdds-insol-next', '@salutejs/plasma-new-hope']
};
````
