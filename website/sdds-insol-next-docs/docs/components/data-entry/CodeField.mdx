...
import { CodeField } from '@salutejs/sdds-insol-next';
...
`import { ONLY_DIGITS_PATTERN, ONLY_CHARS_PATTERN, ONLY_DIGITS_AND_CHARS_PATTERN } from '@salutejs/sdds-insol-next';`
...
Свойство `autoComplete` управляет только нативным автозаполнением и не влияет на явно включенный `enableWebOTP`.
