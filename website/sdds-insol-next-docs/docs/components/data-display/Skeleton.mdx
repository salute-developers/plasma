---
id: skeleton
slug: /components/skeleton
title: Skeleton
description: "Индикация загрузки контента — скелетон-заглушка."
---

import { PropsTable, Description } from '@site/src/components';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Skeleton
Набор компонентов для создания скелетов загрузки (placeholders).

### Тип анимации

Тип анимации задаётся с помощью свойства `animationType`.

| Значение  | Описание                        |
|-----------|---------------------------------|
| `shimmer` | Бегущий градиент (по умолчанию) |
| `pulse`   | Пульсация фона                  |

<Tabs>
    <TabItem value="shimmer" label="shimmer" default>
        ```tsx live
        import React from 'react';
        import { LineSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            return (
                <LineSkeleton size="h3" animationType="shimmer" />
            );
        }
        ```
    </TabItem>
    <TabItem value="pulse" label="pulse">
        ```tsx live
        import React from 'react';
        import { LineSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            return (
                <LineSkeleton size="h3" animationType="pulse" />
            );
        }
        ```
    </TabItem>
</Tabs>

### Длительность анимации

Свойство `animationDuration` позволяет регулировать скорость анимации, переопределяя длительность цикла, заданную по умолчанию.
Значение указывается в миллисекундах.
Значение `0` отключает анимацию.

```tsx live
import React from 'react';
import { LineSkeleton, RectSkeleton, TextSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    return (
        <>
            <LineSkeleton size="h3" animationDuration={1200} />
            <RectSkeleton width="12rem" height="8rem" animationDuration={600} />
            <TextSkeleton size="textM" lines={4} animationDuration={0} />
        </>
    );
}
```

## LineSkeleton
<Description name="LineSkeleton" />

### Быстрый старт

```tsx live
import React from 'react';
import { LineSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    return (
        <LineSkeleton size="h3" />
    );
}
```

## RectSkeleton
<Description name="RectSkeleton" />

### Быстрый старт

```tsx live
import React from 'react';
import { RectSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    return (
        <RectSkeleton width="12rem" height="8rem" />
    );
}
```

## TextSkeleton
<Description name="TextSkeleton" />

### Быстрый старт

```tsx live
import React from 'react';
import { TextSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    return (
        <TextSkeleton size="textM" lines={4} />
    );
}
```

## withSkeleton

HOC `withSkeleton` позволяет применить анимацию скелетона к любому компоненту.
При `skeleton={true}` заменяется фоновый цвет компонента, добавляется градиент, а текст становится прозрачным.

### Быстрый старт

```tsx live
import React from 'react';
import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    const ButtonSkeleton = withSkeleton(Button);

    return (
        <ButtonSkeleton
            fullWidth
            text="Загрузка..."
            view="primary"
            size="s"
            scaleOnInteraction={false}
            outlined={false}
            skeleton={true}
            tabIndex={-1}
        />
    );
}
```

### Тип анимации

Тип анимации задаётся через проп `animationConfig` с обязательным полем `type`.

<Tabs>
    <TabItem value="shimmer" label="shimmer" default>
        ```tsx live
        import React from 'react';
        import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            const ButtonSkeleton = withSkeleton(Button);

            return (
                <ButtonSkeleton
                    text="Загрузка..."
                    view="default"
                    size="s"
                    skeleton={true}
                    animationConfig={{ type: 'shimmer' }}
                />
            );
        }
        ```
    </TabItem>
    <TabItem value="pulse" label="pulse">
        ```tsx live
        import React from 'react';
        import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            const ButtonSkeleton = withSkeleton(Button);

            return (
                <ButtonSkeleton
                    text="Загрузка..."
                    view="default"
                    size="s"
                    skeleton={true}
                    animationConfig={{ type: 'pulse' }}
                />
            );
        }
        ```
    </TabItem>
</Tabs>

### Длительность анимации

Поле `duration` в `animationConfig` задаёт длительность одного цикла анимации в миллисекундах.
Значение `0` отключает анимацию.

```tsx live
import React from 'react';
import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

export function App() {
    const ButtonSkeleton = withSkeleton(Button);

    return (
        <ButtonSkeleton
            text="Загрузка..."
            skeleton={true}
            animationConfig={{
                type: 'shimmer',
                duration: 1000,
            }}
        />
    );
}
```

### Кастомные цвета

Поля `customGradientColor` (для `shimmer`) и `customFadeInColor` / `customFadeOutColor` (для `pulse`) позволяют задать произвольные цвета анимации.

<Tabs>
    <TabItem value="shimmer-color" label="shimmer" default>
        ```tsx live
        import React from 'react';
        import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            const ButtonSkeleton = withSkeleton(Button);

            return (
                <ButtonSkeleton
                    text="Загрузка..."
                    view="default"
                    size="s"
                    skeleton={true}
                    animationConfig={{
                        type: 'shimmer',
                        customGradientColor: 'linear-gradient(90deg, #e0e0e0 25%, #f5f5f5 50%, #e0e0e0 75%)',
                    }}
                />
            );
        }
        ```
    </TabItem>
    <TabItem value="pulse-color" label="pulse">
        ```tsx live
        import React from 'react';
        import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

        export function App() {
            const ButtonSkeleton = withSkeleton(Button);

            return (
                <ButtonSkeleton
                    text="Загрузка..."
                    view="default"
                    size="s"
                    skeleton={true}
                    animationConfig={{
                        type: 'pulse',
                        customFadeInColor: '#e0e0e0',
                        customFadeOutColor: '#f5f5f5',
                    }}
                />
            );
        }
        ```
    </TabItem>
</Tabs>

### Доступность

При использовании `withSkeleton` необходимо воспользоваться атрибутами WAI ARIA. Пример с кнопкой:

```tsx
import React, { useState } from 'react';
import { Button, withSkeleton } from '@salutejs/sdds-insol-next';

const ButtonSkeleton = withSkeleton(Button);

export default function App() {
    const [skeleton, setSkeleton] = useState(false);

    return (
        <ButtonSkeleton
            type="button"
            text={skeleton ? 'Загрузка' : 'Нажмите'}
            skeleton={skeleton}
            aria-busy={skeleton}
            onClick={() => setSkeleton((prevValue) => !prevValue)}
        />
    );
}
```

При `skeleton={true}` и `aria-busy={true}` screen-reader оповещает пользователя о соответствующем статусе компонента.

## API

### LineSkeleton

<PropsTable name="LineSkeleton" />

### RectSkeleton

<PropsTable name="RectSkeleton" />

### TextSkeleton

<PropsTable name="TextSkeleton" />
