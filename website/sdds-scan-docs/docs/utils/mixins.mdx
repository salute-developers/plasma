...
import { addFocus } from '@salutejs/sdds-scan';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_scan`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-scan';
...
###
```tsx
import { addScrollbar } from '@salutejs/sdds-scan';
