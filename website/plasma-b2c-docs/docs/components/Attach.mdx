...
<Tabs>
    ...
    <TabItem value="complex" label="Сложный пример">
        ```tsx live
        import React, { useState } from 'react';
        import { Attach, Button, InformationWrapper } from '@salutejs/plasma-b2c';
        ...
        function App() {
            ...
            return (
                <>
                    {!isLoading && (
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px', padding: '16px 18px', border: '1px dashed black' }}>
