...
title: Avatar
description: "Отображение изображения профиля пользователя, инициалов или запасной иконки."
---
...
### Размер аватара
Размер задается с помощью свойства `size`. Возможные значения свойства: `"xxl"`, `"xl"` `"l"`, `"m"`, `"s"`, `"fit"`, `"scalable"`.

:::caution Устаревшее значение
Значение `"fit"` устарело. Используйте `"scalable"` вместо него.
:::

```tsx live
import React from 'react';
import { Avatar } from '@salutejs/sdds-dfa';

export function App() {
    return (
        <>
            <Avatar size="xxl" url="https://avatars.githubusercontent.com/u/1813468?v=4" />
            <Avatar size="xl" url="https://avatars.githubusercontent.com/u/1813468?v=4" />
