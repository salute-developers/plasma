...
Значения `clear` и `default` для свойства `view` устарели и будут удалены в ближайшее время! Используйте `none` вместо `clear` и `primary` вместо `default`.
:::
...
<Tabs>
    <TabItem value="uncontrolled" label="Uncontrolled" default>
        ...
        import { SegmentGroup, SegmentItem, SegmentProvider, useSegment } from '@salutejs/sdds-dfa';
...
<Tabs>
    ...
    <TabItem value="size" label="Size">
        ...
        ```tsx live
        import React from 'react';
        import { SegmentGroup, SegmentItem, SegmentProvider } from '@salutejs/sdds-dfa';
