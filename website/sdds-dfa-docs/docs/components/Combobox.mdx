...
<Tabs>
    ...
    <TabItem value="chipsPlaceholder" label="Placeholder with chips">
        В режиме множественного выбора placeholder можно скрыть, когда выбран хотя бы один чип.
    ...
    <TabItem value="predefined" label="Predefined">
        Есть возможность задать значения по дефолту. Также можно управлять состоянием снаружи компонента.
    ...
    <TabItem value="slots" label="beforeList / afterList">
        ...
        ```tsx live
        import React, { useState } from 'react';
        import { BodyXS, Combobox, Divider, LinkButton, TabItem, Tabs } from '@salutejs/sdds-dfa';
...
Размер Combobox задаётся с помощью свойства `size`. Возможные значения свойства: `"xl"`, `"l"`, `"m"`, `"s"` или `"xs"`:
...
export function App() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '350px', height: '400px' }}>
            <Combobox size="xl" items={items} label="Label" placeholder="xl" />
            <Combobox size="l" items={items} label="Label" placeholder="l" />
            <Combobox size="m" items={items} label="Label" placeholder="m" />
            <Combobox size="s" items={items} label="Label" placeholder="s" />
            <Combobox size="xs" items={items} label="Label" placeholder="xs" />
        </div>
    );
}
```
...
```tsx live
    import React, { useState } from 'react';
    import { Combobox, ComboboxItemOption } from '@salutejs/sdds-dfa';
