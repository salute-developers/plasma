...
<Tabs>
    ...
    <TabItem value="predefined" label="Predefined">
        ...
        ```tsx live
        import React from 'react';
        import { Select, Button } from '@salutejs/sdds-dfa';
    ...
    <TabItem value="renderItem" label="Render item">
        ...
        ```tsx live
        import React from 'react';
        import { Select, Cell } from '@salutejs/sdds-dfa';
    ...
    <TabItem value="slots" label="beforeList / afterList">
        `beforeList` и `afterList` добавляют контент в начало и конец списка.
        ...
        ```tsx live
        import React, { useState } from 'react';
        import { BodyXS, Divider, LinkButton, Select, TabItem, Tabs } from '@salutejs/sdds-dfa';
        ...
        export function App() {
            ...
            return (
                <div style={{ display: 'flex', flexDirection: 'column', maxWidth: '350px', height: '420px' }}>
                    <Select
                        multiselect
                        items={items}
                        value={value}
                        ...
                        listMaxHeight="200px"
                        beforeList={
                            <Tabs view="divider" size="xs" stretch>
                                {regions.map((item) => (
                                    <TabItem
                                        key={item.value}
                                        view="secondary"
    ...
    <TabItem value="sortItems" label="Сортировка">
        Свойство `sortItems` принимает компаратор `(a, b) => number`.
        Сортируются только элементы первого уровня: порядок вложенных элементов и исходный массив `items` не изменяются.
        Без `sortItems` элементы отображаются в исходном порядке.

        В примере выбранные элементы перемещаются наверх при каждом изменении выбора.
...
Размер Select задаётся с помощью свойства `size`. Возможные значения свойства: `"xl"`, `"l"`, `"m"`, `"s"` или `"xs"`:
...
export function App() {
    const items = [
        { value: 'opt_1', label: 'Опция 1' },
        { value: 'opt_2', label: 'Опция 2' },
        { value: 'opt_3', label: 'Опция 3' },
    ];

    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '350px', height: '400px' }}>
            <Select size="xl" items={items} label="Label" placeholder="xl" />
            <Select size="l" items={items} label="Label" placeholder="l" />
            <Select size="m" items={items} label="Label" placeholder="m" />
            <Select size="s" items={items} label="Label" placeholder="s" />
            <Select size="xs" items={items} label="Label" placeholder="xs" />
        </div>
    );
}
```
...
```tsx live
import React, { useState } from 'react';
import { Select, TextField } from '@salutejs/sdds-dfa';
