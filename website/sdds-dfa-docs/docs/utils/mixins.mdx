...
import { addFocus } from '@salutejs/sdds-dfa';
...
Позволяет создать базовый стиль подложки для любого блока, со следующим набором свойств, значения которых доступны из темы `sdds_dfa`:
...
```tsx
import { applyPaper } from '@salutejs/sdds-dfa';
